...
class RelatedSearch:
    ...
    def concatinateFields(customer_id, dataset_id, type_of_tickets, incident_number=0):
        ...
        try:
            ...
            logging.error("Error occured: Hint: 'inputFieldList' is not defined with correct fields in 'config/intent.ini' file.--%s"% str(e))
        ...
        try:
            if (type_of_tickets == 'closed'):
                if (incident_number == 0):  # Generate TF for all corpus
                    ...
                    #     {'CustomerID': customer_id, "DatasetID": dataset_id, ticket_id_field_name: incident_number},
                    #     {ticket_id_field_name: 1, '_id': 0,
                    #      input_field_list[0]: 1, assignment_group:1})))
    ...
    def create_word2vec_model(descriptions, use_stop_list=False):
        ...
        # def create_word2vec_model(descriptions, save_path, use_stop_list=False):
        """
        Arguments:
        ...
        # model.build_vocab(descr_tokens, progress_per=1000)
        # model.train(descr_tokens,total_examples=model.corpus_count,epochs=10)
        # word2vec_save_path = save_path +"\word2vec.model"
        # model.save(word2vec_save_path)
        return model

    @staticmethod
    def create_doc2vec_model(descriptions, use_stop_list=False):
        # create_doc2vec_model(descriptions,save_path, use_stop_list=False):
        """
        Arguments:
        ...
        # doc2vec_save_path = save_path +"\d2v.model"
    ...
    def getRelatedTickets(customer_id, dataset_id, incident_number,search_alogorithm,rt_assignment_group=None,rt_created_by=None):
        try:
            ...
            try:
                ...
                logging.error("Error occured: Hint: 'displayFieldList' is not defined correctly  in 'config/intent.ini' file.with details of %s" %str(e))
            ...
            # print("final result issss",fin_result_list)
            # print("length of finalresult" ,len(fin_result_list))
                        
            if len(fin_result_list) > 0:
                #print("Related tickets for ticket"+incident_number+"are",result_list)
                related_incid_list=[]
                for each in fin_result_list:
                    print(each[ticket_id_field_name])
                    related_incid_list.append(each[ticket_id_field_name])
                # print("final inicdent list ",related_incid_list)
                return related_incid_list
                #return json_util.dumps(related_incid_list)
            else:
