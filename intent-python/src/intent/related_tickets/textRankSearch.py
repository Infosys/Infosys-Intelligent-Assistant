...
class TextRankSearch:
    ...
    def concatinateFields(customer_id, dataset_id, type_of_tickets, incident_number=0):
        ...
        try:
            ...
            logging.error('%s: Error occured in Concatinating input fields using textRankAlgorithm: %s' % (TextRankSearch.timestamp(), str(e)))
    ...
    def get_raw_text(customer_id, dataset_id, field_name, incident_number, type_of_tickets = 'closed'):
        try:
            ...
            logging.info(" %s Key not found in Tbl Incident Training while working with get_raw_text- %s"  % (RelatedTicket.timestamp(), str(e)))
            raise
    ...
    def getRelatedTickets(customer_id, dataset_id,incident_number):
        try:
            ...
            logging.info("%s exception occured in textRank_getRelatedTickets Method for a Incident -%s with exception details as - %s" % (RelatedSearch.timestamp(),incident_number, str(e)))
            raise
    ...
    def getRelatedOpenTickets(customer_id, dataset_id, incident_number):
        try:
            ...
            logging.info("%s exception occured in textRank_getRelatedOpenTickets Method for a Incident -%s with exception details as - %s" % (RelatedSearch.timestamp(),incident_number, str(e)))
            raise
