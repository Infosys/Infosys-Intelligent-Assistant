...
class relatedticket(object):
    ...
    def startProcess(customer_id):
        ...
        realTimeincidents=list(MongoDBPersistence.rt_tickets_tbl.find({"user_status" : {"$in":["Not Approved"]}},{ticket_id_field_name:1,'_id':0,group_field_name:1,'opened_by':1,"CustomerID":1, "DatasetID" :1 }))
        realTimeApprovedincidents=list(MongoDBPersistence.rt_tickets_tbl.find({"state": {"$nin": ["closed"]}, "user_status":{"$in":["Approved"]}},{ticket_id_field_name:1,'_id':0,group_field_name:1,'opened_by':1,"CustomerID":1, "DatasetID" :1 }))
                ...
                if group_field_name and 'createdby' in realTimeincidents_updated[0]:
                    for num in realTimeincidents_updated:
                        try:
                            ...
                            ticketflag = MongoDBPersistence.related_tickets_tbl.find_one({"RT_Ticket_Number":incident_number,"Algorithm_name":search_algorithm})
                            if ticketflag:
                                logging.info("Related ticket with %s for this %s ticket number is presented" % (search_algorithm,incident_number))
                                continue
                            else:
                            ...
                            logging.info("Skipped this %s incident because of following exception-  %s  " % (num[ticket_id_field_name],str(e)))
                            continue 
                elif group_field_name in realTimeincidents_updated[0] and 'createdby' not  in realTimeincidents_updated[0]:
                    for num in realTimeincidents_updated:
                        try:
                            incident_number=(num[ticket_id_field_name])
                            rt_assignemnt_group=(num[group_field_name])
