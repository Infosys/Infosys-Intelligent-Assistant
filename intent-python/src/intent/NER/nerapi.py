...
class NER(object):
    ...
    def alertEnableStatus(sms_status, whatsapp_status, call_status,ms_teams_enabled):
        try:
            # log_setup()
            MongoDBPersistence.assign_enable_tbl.update_one({},{'$set':{"sms_enabled" :sms_status ,  "whatsapp_enabled" : whatsapp_status, 'call_enabled': call_status,"msteams_enabled" :ms_teams_enabled }},upsert=True)
        except Exception as e:
            logging.info('%s: Error: %s'%(RestService.timestamp,str(e)))
            return 'Failure'
        return 'Success'
    ...
    def TwilioEnableStatus(twilio_enabled):
        try:
            # log_setup()
            MongoDBPersistence.assign_enable_tbl.update_one({},{'$set':{"twilio_enabled" :twilio_enabled }},upsert=True)
        except Exception as e:
            logging.info('%s: Error: %s'%(RestService.timestamp,str(e)))
            return 'Failure'
        return 'Success'
            ...
            if (new_tag=='true'):
                ...
                for item in new_items:
                    if item['entity'] in existing_list:
                        existing_words = MongoDBPersistence.annotation_mapping_tbl.find_one({ 'entity': item['entity']},{"description" :1, '_id':0})['description']
                        existing_words.extend(item["description"])
                        new_words_list = list(set(existing_words))
                        
                        MongoDBPersistence.annotation_mapping_tbl.update_one({'entity': item['entity']}, {"$set":{"description": new_words_list}}, upsert=False)
                    else:
        ...
        try:
            ...
            if len(topic_list) >= num_of_topics:
               ...
               return json_util.dumps({'response':topic_list[:num_of_topics]})
                ...
                lda_model_tfidf = gensim.models.LdaModel(corpus_tfidf, num_topics=num_of_topics, id2word=dictionary, passes=2)
    ...
    def exportNerConfigtoCSV():
        try:
            ...
            #return json_util.dumps(combined_df_list[0].keys())
            #csv_data_format.append(combined_df_list[0].keys())
            #print(csv_data_format)
    ...
    def saveKnowledgeInfo():
        try:
            ...
            # mapping_details = request.get_json()
            ...
            if mapping_dict:
                MongoDBPersistence.knowledge_entity_tbl.update_one({"Mapping":True},{"$set": {"Mapping":True,  "MappingIiaList" :list(mapping_dict.keys()), "AccountToIiaColumnMapping":mapping_dict}}, upsert=True)
    ...
    def getKnowledgeInfo():
        try:
            ...
            knowledge_info_lst = list(MongoDBPersistence.knowledge_entity_tbl.find({'Mapping': {'$exists': False}},{'_id':0}))
    ...
    def getKnowledgeEntityInfo(number,entity_name):
        try:
            ...
            #knowledge_info_lst = list(MongoDBPersistence.knowledge_entity_tbl.find({},{'_id':0}))
            knowledge_entity_info = MongoDBPersistence.predicted_tickets_tbl.find_one({'number' : number},{'_id':0, "KgInfo" :1})
    ...
    def getKnowledgeInfoColumnNames():
        try:
            ...
            mapping_list = MongoDBPersistence.knowledge_entity_tbl.find_one({"Mapping" : True},{"MappingIiaList" :1,'_id' : 0 })
            if mapping_list:
                iia_column_headers = mapping_list['MappingIiaList']
            else:
                iia_column_headers = ["Server", "Application", "Contact Name", "Platforms", "Service Name"  ]
                # print("'MappingIiaList' is not defined Taking default as list")
                logging.info("%s 'MappingIiaList' is not defined Taking default as list as  [Server, Application, Contact Name, Platforms, Service Name ]'.." %RestService.timestamp())
            response = {}
