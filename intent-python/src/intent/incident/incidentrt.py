...
class IncidentRT(object):
    ...
    #@app.route('/insert/<int:customer_id>', methods=["POST"])
    @staticmethod
    def db_insert_input_tickets( customer_id):
        #print('Inside Insert')
        #tickets_ = db["TblIncidentRT"]
        #csv file of users
        #f = self.request.body
        try:
    ...
    #latin-1
        ...
        if (custom_incidentrt_flag=='failure'):
            ...
            api = custom_url + "api/custom_insertRT/"+str(customer_id)  # customer id hardcoded
            ...
            return resp
            #return (result1)
            #return redirect(url_for('predict', customer_id=customer_id))
            #return ('File uploaded successfully! Click "Predict" to continue.')
        
        
    #@app.route("/updateRT/<int:customer_id>/<incident_number>/<dataset_name>",methods=["PUT"])
    @staticmethod
    def update_RT( customer_id, incident_number, dataset_name):
    ...
    #@app.route("/tickets/<int:customer_id>/<int:dataset_id>",methods=["GET"])
    @staticmethod
    def getTickets( customer_id,dataset_id):
    ...
    #@app.route("/tickets/<int:customer_id>/<int:dataset_id>/<ticket_id>",methods=["GET"])
    @staticmethod
    def getRawData( customer_id,dataset_id,ticket_id):
        #rt_tickets_tbl is a pymongo object to TblIncidentRT
        # field_mapping = MappingPersistence.get_mapping_details(customer_id)
    ...
    #@app.route('/exportPredToCSV/<int:customer_id>', methods=["GET"])
    @staticmethod
    def exportPredToCSV( customer_id):
        # field_mapping = MappingPersistence.get_mapping_details(customer_id)
        # group_field_name = field_mapping['Group_Field_Name']
        ...
        try:
            ...
            #a = list(predicted_tickets_tbl.find({"CustomerID": customer_id,"DataSetName":dataset_name},{'_id':0}))
        ...
        #combined_df = pd.concat([pred_tkts_df, rt_tkts_df], axis=1)
