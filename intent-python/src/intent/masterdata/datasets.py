...
class DatasetMasterData(object):
    ...
    def guessDataset(tickets, customer_id):
        # try:
        #     MongoDBPersistence.rt_tickets_tbl.delete_many({"CustomerID":customer_id})
        #     logging.info("%s Records deleted successfully.."%RestService.timestamp())
        # except:
        #     logging.info("%s Some error occured in deleting the records from rt table"%RestService.timestamp())
        ...
        try:
            ...
            if(uniqueFlag.lower() == "no"):
                #default_dataset_id = MongoDBPersistence.training_tickets_tbl.find_one({"CustomerID":customer_id})["DatasetID"]
        ...
        if (login_user):
            ...
            for ticket in tickets:
                dataset_id = 0 #initial
                if(unique_field_flag):
                    logging.info("Unique field is not required , Going for default dataset id %s"%(default_dataset_id))
                    ...
                    for dataset in datasets:
                        ...
                        for field in fields:
                            try:
                                ...
                                logging.info('%s: distinct values of %s are %s'%(RestService.timestamp(),field['FieldName'],dataset_values))
                            except Exception as e:
                            ...
                            if(field['FieldName'] in ticket.keys()):
                                logging.info('%s: real time ticket with number %s %s value is %s'%(RestService.timestamp(),ticket[ticket_id_field_name],field['FieldName'],ticket[field['FieldName']]))
                ...
                if(dataset_id == 0):
                    logging.info('%s: the ticket (Incident Number: %s) do not match with any of the datasets.'%(RestService.timestamp(),ticket[ticket_id_field_name]))
                ...
                if(dataset_info_dict):
                    ...
                    logging.info('%s: For a given customer, FieldSelections not found in the record. Please train the models with appropriate dataset to predict your ticket fields.'%RestService.timestamp())
                    return "failure"
                ...
                for pred_field in pred_field_list:
                    ...
                    #print(list(set(input_field_list) & set(csv_df_cols))[::-1])
                    if(set(input_field_list) == set(set(input_field_list) & set(ticket.keys()))):
                        logging.info('%s: All required input customer choices for %s are there with csv data.'%(RestService.timestamp(),pred_field))
                        continue
                    else:
                        logging.error('%s: Incomplete input data in the ticket for a prediction field %s'%(RestService.timestamp(),pred_field))
                try:
                    logging.info('%s: Trying to insert tickets into TblIncidentRT...'%RestService.timestamp())
                    # MongoDBPersistence.rt_tickets_tbl.insert_one(ticket)
                    MongoDBPersistence.rt_tickets_tbl.update_one({ticket_id_field_name:ticket[ticket_id_field_name]},{'$set': ticket}, upsert=True)
                    #logging.info('%s: ticket inserted successfully'%RestService.timestamp())
                except Exception as e:
    ...
    def priorityList( customer_id, dataset_id):
        # log_setup()
        priority_List = list(MongoDBPersistence.training_tickets_tbl.find({"CustomerID":customer_id, "DatasetID":dataset_id}).distinct('priority'))
        if(priority_List):
    ...
    #@app.route('/allValues/<int:customer_id>/<int:dataset_id>/<predicted_field>', methods=["GET"])
    @staticmethod
    def allValues( customer_id, dataset_id, predicted_field):
        # log_setup()
        all_values = list(MongoDBPersistence.training_tickets_tbl.find({"CustomerID":customer_id, "DatasetID":dataset_id}).distinct(predicted_field))
        if(all_values):
            #Return array of distinct values
    ...
    #@app.route('/allDatasetNames/<int:customer_id>', methods=["GET"])
    @staticmethod
    def allDatasetNames( customer_id):
        # log_setup()
        all_datasetNames = list(MongoDBPersistence.datasets_tbl.find({"CustomerID":customer_id}).distinct('DatasetName'))
        if(all_datasetNames):
            #Return array of distinct values
    ...
    #@app.route("/saveFieldSelections/<int:customer_id>/<int:dataset_id>",methods=["PUT"])
    @staticmethod
    def saveFieldSelections( customer_id, dataset_id):
        index_count = 0
        ...
        o_fieldselection_doc =  MongoDBPersistence.datasets_tbl.find_one({'CustomerID': customer_id, "DatasetID":dataset_id},{'FieldSelections':1, '_id':0, 'DependedPredict':1})
        if(o_fieldselection_doc):
            ...
            for fieldselection_dict in o_fieldselection_list:
                if(fieldselection_dict['FieldsStatus']=='Approved') or (fieldselection_dict['FieldsStatus']=='InProgress'):
            ...
            o_depended_list.append(DependedPredict)
        try:
            MongoDBPersistence.datasets_tbl.update_one({'CustomerID': customer_id, "DatasetID":dataset_id}, {"$set": {"FieldSelections":n_fieldselection_list, "DependedPredict": o_depended_list}}, upsert=True)
            #First time when user uploading dataset, He will have to select UniqueFields that identify him
            if(data["UniqueFields"]):
                MongoDBPersistence.datasets_tbl.update_one({'CustomerID': customer_id, "DatasetID":dataset_id}, {"$set": {"UniqueFields":data["UniqueFields"]}}, upsert=False)
            ...
            data_balance = pd.DataFrame(list(MongoDBPersistence.training_tickets_tbl.find({'CustomerID': customer_id, "DatasetID":dataset_id},{"_id":0})))
            output_data = MongoDBPersistence.datasets_tbl.find_one({'CustomerID': customer_id, "DatasetID":dataset_id},{'FieldSelections':1, '_id':0})
            ...
            if(data_below_threshold_list):
                if len(data_below_threshold_list)<5:
                    resp = {'Status':'success','Classes having less number of tickets':data_below_threshold_list,'Warning':True, 'Message':'The above mentioned classes do not have required minimum tickets for Training. Please add more tickets for corresponding classes'}
                elif len(data_below_threshold_list)>5:
                    resp = {'Status':'success','Warning':True,'Classes having less number of tickets':data_below_threshold_list[0:5], 'Message':'Many classes do not have required minimum tickets for Training. Please add more tickets for corresponding classes.\n Refer EDA advanced report for more details'}
            else:
    ...
    #@app.route("/updateRetrainPreference/<int:customer_id>/<int:dataset_id>/<status>", methods=["PUT"])
    @staticmethod
    def updateRetrainPreference(customer_id,dataset_id,status): #status = Approved/Discarded
        data = request.get_json()  
        # log_setup()
        #data = [{'PredictedFieldName': 'assignment_group', 'AlgorithmName': 'RandomForestClassifier'}]
        o_fieldselection_list =  MongoDBPersistence.datasets_tbl.find_one({'CustomerID':customer_id,"DatasetID":dataset_id},{'DatasetName':1,'FieldSelections':1, '_id':0})
            ...
            if(status=="Approved"):
                ...
                for fieldselection_dict in o_fieldselection_list:
                    if(fieldselection_dict['FieldsStatus']=='Approved'):
                        ...
                        #print("updated_selections:%s"%updated_selections) 
                ...
                FieldSelections.append(FieldSelections_dict)
                    #print("FieldSelections:%s"%FieldSelections)
                ...
                logging.info('%s: moved previous "Approved" to "Archived" pkls & amking current Retraining as Approved.'%RestService.timestamp())
                ...
                for filename in os.listdir("data"):
                    ...
                    if(customer_name in file_list):
                        if(dataset_name in file_list):
                            if("Retraining" in file_list):
                                os.remove("data/"+filename)
                logging.info('%s: Pickle files for current selections deleted successfully.. Also updated the TblTraining.'%RestService.timestamp())
                return 'success'
        ...
        last_training_dict = MongoDBPersistence.training_hist_tbl.find_one({"CustomerID" : customer_id,"DatasetID" : dataset_id,\
        "TrainingStatus" : "Retraining", "TrainingID": {"$exists": True}},{'_id':0,"TrainingID":1}, sort=[("TrainingID", -1)])
        if(last_training_dict):
            last_training_id = last_training_dict['TrainingID']
        else:
            #It should be non-Registered Customer
            logging.info('%s: Failure: Last InProgress training info not found for the customer.'%RestService.timestamp())
        ...
        try:
            ...
            "TrainingStatus" : "Approved"}, {"$set": {"TrainingStatus":"Archived"}}, upsert=False)

            MongoDBPersistence.training_hist_tbl.update_one({'CustomerID':customer_id,"TrainingID":last_training_id,\
            "DatasetID" : dataset_id}, {"$set": {"TrainingStatus":status}}, upsert=False)
            algo_name = data[0]['AlgorithmName']
            print("algo_name:", algo_name)
            best_algo_score = MongoDBPersistence.training_hist_tbl.find_one({"CustomerID":customer_id,"DatasetID":dataset_id,"TrainingStatus":status},{'PredictedFields':1,"AlgorithmParameters":1,'_id':0})
            ...
            MongoDBPersistence.datasets_tbl.update_one({'CustomerID':customer_id,"DatasetID":dataset_id},{"$set": {"BestAlgoParams":update_dict}}, upsert=True)
    ...
    #@app.route("/updateUserPreference/<int:customer_id>/<int:dataset_id>/<status>",methods=["PUT"])
    @staticmethod
    def updateUserPreference( customer_id,dataset_id,status): #status = Approved/Discarded
        ...
        o_fieldselection_list =  MongoDBPersistence.datasets_tbl.find_one({'CustomerID':customer_id,"DatasetID":dataset_id},{'DatasetName':1,'FieldSelections':1, 'DependedPredict': 1, '_id':0})
        customer_name = MongoDBPersistence.customer_tbl.find_one({'CustomerID':customer_id},{'CustomerName':1, '_id':0})
        customer_name = customer_name['CustomerName']
        if(o_fieldselection_list):
            ...
            if(status=="Approved"):
                ...
                for fieldselection_dict in o_fieldselection_list:
                    if(fieldselection_dict['FieldsStatus']=='InProgress' or fieldselection_dict['FieldsStatus']== 'Approved'):
                        ...
                        if(customer_name in file_list):
                            if(dataset_name in file_list):
                                if('InProgress' in filename.split('__')):
                                    n_filename = filename.replace("InProgress","Approved")
                                    os.rename("data/"+filename,"data/"+n_filename)
                    logging.info('%s: moved previous "Approved" to "Archived" pkls & amking current InProgress as Approved.'%RestService.timestamp())
                else:
                    logging.info('%s: Selected choices have been saved successfully.'%RestService.timestamp())
                    logging.info('%s: Updating TblCustomer & TblTraining for the selected choices.'%RestService.timestamp())
                    MongoDBPersistence.datasets_tbl.update_one({'CustomerID':customer_id,"DatasetID" : dataset_id}, {"$set": {"FieldSelections":FieldSelections}}, upsert=False)
                    logging.info('%s: Updated: TblDataset'%RestService.timestamp())
                    return 'success'
                    #return 'success'
            elif(status=="Discarded"):
        ...
        #Update Training status
        #Recieving last training ID to update TblTraining 
        last_training_dict = MongoDBPersistence.training_hist_tbl.find_one({"CustomerID" : customer_id,"DatasetID" : dataset_id,"TrainingStatus" : "InProgress", "TrainingID": {"$exists": True}},{'_id':0,"TrainingID":1}, sort=[("TrainingID", -1)])
        if(last_training_dict):
            last_training_id = last_training_dict['TrainingID']
        else:
            #It should be non-Registered Customer
        ...
        try:
            logging.info('%s: Updating TblCustomer & TblTraining for the selected choices.'%RestService.timestamp())
            MongoDBPersistence.datasets_tbl.update_one({'CustomerID':customer_id,"DatasetID" : dataset_id}, {"$set": {"FieldSelections":FieldSelections, 'DependedPredict':o_depended_list}}, upsert=False)
            logging.info('%s: Updated: TblDataset'%RestService.timestamp())
            MongoDBPersistence.training_hist_tbl.update_one({'CustomerID':customer_id,"DatasetID" : dataset_id,"TrainingStatus" : "Approved"}, {"$set": {"TrainingStatus":"Archived"}}, upsert=False)
            MongoDBPersistence.training_hist_tbl.update_one({'CustomerID':customer_id,"TrainingID":last_training_id,"DatasetID" : dataset_id}, {"$set": {"TrainingStatus":status}}, upsert=False)
            logging.info('%s: Updated: TblTraining'%RestService.timestamp())
            resp = 'success'
        except Exception as e:
            logging.error('%s: Error occurred: %s '%(RestService.timestamp(),str(e)))
    ...
    #@app.route('/getDatasetID/<int:customer_id>/<int:team_id>', methods=["GET"])
    @staticmethod
    def getDatasetID( customer_id, team_id):
        # log_setup()
        dataset_ = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"TeamID":team_id},{"DatasetID":1,"_id":0})
        if(dataset_):
    ...
    #@app.route('/getDatasetDetails/<int:customer_id>/<int:dataset_id>', methods=["GET"])
    @staticmethod
    def getDatasetDetails( customer_id, dataset_id):
        ...
        train_dict = {}
        #dataset_dict = datasets_tbl.find_one({"CustomerID": customer_id,"DatasetID":dataset_id},{"DatasetName":1,"FieldSelections":{"$exists": True},"_id":0})
        dataset_dict = MongoDBPersistence.datasets_tbl.find_one({"CustomerID": customer_id,"DatasetID":dataset_id},{"DatasetName":1,"ColumnNames":1,"TicketCount":1, "training_mode":1, "_id":0})
        ...
        if(dataset_dict):
            ...
            #if 'description' not in dataset_dict['ColumnNames']:
            #    dataset_dict['ColumnNames'].append('description')
            ...
            #train_dict['field_selections'] = dataset_dict["FieldSelections"]
        else:
            logging.info('%s: Dataset details not found for the team.'%RestService.timestamp()) 
        #dataset = pd.DataFrame(list(MongoDBPersistence.training_tickets_tbl.find({'CustomerID': customer_id, "DatasetID":dataset_id}, {"CustomerID":0, "_id":0, "DatasetID":0, "TrainingFlag":0})))
    ...
    #@app.route('/deleteDataset/<int:customer_id>/<int:dataset_id>', methods=["DELETE"])
    @staticmethod
    def deleteDataset( customer_id,dataset_id):
        ...
        try:
            ...
            MongoDBPersistence.teams_tbl.update_many({"CustomerID": customer_id, 'DatasetID':dataset_id}, {"$unset": {"DatasetID": 1}})
        ...
        try:
            ...
            logging.info('%s: Related Tickets models and Preprocessed dataset has been deleted successfully from Related_Ticket_Model folder.'%RestService.timestamp())
        except Exception as e:
            logging.error('%s: Error occurred: %s '%(RestService.timestamp(),str(e)))
        return resp
    
    @staticmethod
    def updateDaemonTraining(customer_id,dataset_id, training_completed_flag, completed_status, training_start_date, training_end_date = ''):
        training_stats = {}
        # log_setup()
        ...
        try:
            MongoDBPersistence.datasets_tbl.update_one({"CustomerID": customer_id, 'DatasetID':dataset_id}, {"$set": {"TrainingStatus": training_stats}})
            resp = 'success'
        except Exception as e:
            logging.error('%s: Error occurred: %s '%(RestService.timestamp(),str(e)))
            resp = 'failure'
    ...
    def getTagsDatasetDetails( customer_id, dataset_id):
        ...
        # log_setup()
        #dataset_dict = datasets_tbl.find_one({"CustomerID": customer_id,"DatasetID":dataset_id},{"DatasetName":1,"FieldSelections":{"$exists": True},"_id":0})
        dataset_dict = MongoDBPersistence.tag_dataset_tbl.find_one({"CustomerID": customer_id,"TagsDatasetID":dataset_id},{"TagsDatasetName":1,"ColumnNames":1,"TicketCount":1,"_id":0})
        if(dataset_dict):
    ...
    @staticmethod
    def getTagDatasetID( customer_id, team_id):
        dataset_ = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"TeamID":team_id},{"TagsDatasetID":1,"_id":0})
        # log_setup()
        if(dataset_):
    ...
    def deleteTagDataset( customer_id,dataset_id):
        ...
        try:
            ...
            MongoDBPersistence.teams_tbl.update_many({"CustomerID": customer_id, 'TagsDatasetID':dataset_id}, {"$unset": {"TagsDatasetID": 1}})
            
            MongoDBPersistence.tag_training_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            # MongoDBPersistence.rt_tickets_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            # MongoDBPersistence.predicted_tickets_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            
            # MongoDBPersistence.training_hist_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            
            # MongoDBPersistence.resource_details_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            # MongoDBPersistence.applicationDetails_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            # MongoDBPersistence.roaster_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            
            # MongoDBPersistence.tickets_weightage_tbl.delete_many({})

            # MongoDBPersistence.whitelisted_word_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
            # MongoDBPersistence.known_errors_tbl.delete_many({'CustomerID':customer_id,'TagsDatasetID':dataset_id})
    ...
    # @staticmethod
    # def saveTagFieldSelection( customer_id, dataset_id):
    #     data = request.get_json()
    #     o_fieldselection_list =  MongoDBPersistence.datasets_tbl.find_one({'CustomerID': customer_id, "TagsDatasetID":dataset_id},{'TagBasedField':1, '_id':0})
    #     if(o_fieldselection_list):
    #         o_fieldselection_list = o_fieldselection_list['FieldSelections']
    #         n_fieldselection_list = o_fieldselection_list
    #         for fieldselection_dict in o_fieldselection_list:
    #             if(fieldselection_dict['FieldsStatus']=='InProgress'):
    #                 n_fieldselection_list.remove(fieldselection_dict)
    #         n_fieldselection_dict = {"FieldsStatus":'InProgress'}
    #         n_fieldselection_dict["PredictedFields"] = data["FieldSelections"]
    #         n_fieldselection_list.append(n_fieldselection_dict)
    ...
    #         n_fieldselection_list.append(n_fieldselection_dict)
    #     try:
    #         MongoDBPersistence.datasets_tbl.update_one({'CustomerID': customer_id, "DatasetID":dataset_id}, {"$set": {"FieldSelections":n_fieldselection_list}}, upsert=False)
    #         #First time when user uploading dataset, He will have to select UniqueFields that identify him
    #         if(data["UniqueFields"]):
    #             MongoDBPersistence.datasets_tbl.update_one({'CustomerID': customer_id, "DatasetID":dataset_id}, {"$set": {"UniqueFields":data["UniqueFields"]}}, upsert=False)
    #         logging.info('%s: FieldSelections have been saved successfully into TblDataset'%RestService.timestamp())
