...
@app.route(
    '/api/getRosterScreenMappingDetails/<int:customer_id>/<team_name>/<int:currentMonthDays>/<int:page_size>/<int:page_num>')
def getRosterScreenMappingDetails(customer_id, team_name, currentMonthDays, page_size, page_num):
    return ResourceMasterData.getRosterScreenMappingDetails(customer_id, team_name, currentMonthDays, page_size,
                                                            page_num)


@app.route('/api/getRoasterMappingCount', methods=['GET'])
def getRoasterMappingCount():
    return ResourceMasterData.getRoasterMappingCount()
...
class ResourceMasterData(object):
    ...
    # @staticmethod
    # def find_assigneesForAssignmentGroups(assignment_groups):
    ...
    #     assignees = ['']*len(assignment_groups)

    #     #print("Inside assignee groups.2"+ str(assignment_groups))
    #     try:
    #         logging.info('calling find_assignees function for each assignmnet group')
    #         for ag in counts.keys():
    #             res = ResourceMasterData.find_assignees(ag,counts[ag])
    #             if(res):
    #                 j = 0
    #                 for k in range(0 , len(assignment_groups)):
    #                     if assignment_groups[k] == ag:
    ...
    #     @staticmethod
    #     def find_assignees(assignment_group,incidentCount):
    #         logging.info('%s: in "find_assignees" method, assign grp: %s, incident count: %s'%(RestService.timestamp(), assignment_group,incidentCount))
    #         try:
    #             resource_name_lst = []
    #             roaster_lst = []
    #             resource_lst = []
    ...
    #                         tickets_assigned=0
    #                         workload = 0.0
    #                         incident_lst= list(MongoDBPersistence.rt_tickets_tbl.find({"assigned_to":resource_doc['resource_name']},{'workload':1,"_id":0}))
    ...
    #                     if(i!=incidentCount):
    #                         logging.info('%s: some of the assignment group cannot be assigned with assignee becoz, all resources having maximum bandwidth'%RestService.timestamp())
    #                     else:
    ...
    #         except Exception as e:
    #             logging.error("Here is an Exception"%(RestService.timestamp(),str(e)))

    #         return assignments

    # @app.route('/assignIncident/<incident_no>/<int:resource_id>/<assignment_group>')
    @staticmethod
        ...
        if (incident_doc):
            ...
            if (ticket_doc):
                ...
                if (app_doc):
                    ...
                    #    #--getting details from service now--
                    #    response = incidents.get(query={'number':incident_no},fields=['assignment_group','priority','state'])
                    logging.info(
                    ...
                    # incidents.update(query={'number':'INC0010003' },payload ={'state':2})
    ...
    #     logging.info('%s: In "find_possibleassignees" method, argument - assign grp: %s'%(RestService.timestamp(), assignment_group))
    ...
    #     class_name = customer_name +"_"+ "".join(dataset_name.split()) +"_"+ "resources"
    #     module_name = "intent.custommasterdata." + class_name
    #     logging.info("class name formed here is  %s and module is %s in find_possibleassignees"%(class_name,module_name))
    #     try:
    #         module = importlib.import_module(module_name)
    #         class_ = getattr(module, class_name)
    #         response = class_.find_possibleassignees(assignment_group)
    #         logging.info("Custom file called to find possible assignees and returning the response to UI  %s"%(response))
    #         return response
    #     except Exception as e:
    #         logging.error('%s: Error occurred: %s ' % (RestService.timestamp(), str(e)))
    #         logging.info("Error can be ignored, there is no customized find_possibleassignees module uploaded. \
    #         Going for basic method to find possible assignees ")
    ...
    #     logging.info('Trying to call "getRoasterList()" method of "ResourceAvailabilityMasterData" to get "roaster_lst"')
    #     roaster_lst = ResourceAvailabilityMasterData.getRoasterList(assignment_group)
    ...
    #     else:
    #         logging.error('Could not found value Either from "getAppWeightage()" or from "getRoasterList()" for assign group: %s'%assignment_group)
    #     return res_names
    ...
    def resource_info_of_group(assignment_group):
        ...
        try:
            ...
            if (len(roaster_lst) > 0):
                ...
                FMT = '%H:%M:%S'
                # roaster_lst = [dict(t) for t in {tuple(d.items()) for d in roaster_lst}]
                for resource_doc in resource_lst:
                    workload = []
                    availability = False
                    available_till = '__'
                    ...
                    if (email_id in roaster_keys):
                        ...
                        if ('tomo_start_date' in roaster_doc_keys):
                            ...
                            # elif (int(time_diff.split(':')[0]) >= 0): next_available_in = '1 day '
                            else:
    ...
    # @app.route('/findpossibleassignees/<assignment_group>', methods=['GET'])
    @staticmethod
    def findpossibleassignees(incident_number, assignment_group):
    ...
    # @app.route('/uploadResourceDetails/<int:customer_id>/<team_name>', methods=["POST"])
    @staticmethod
    def db_insert_resource_details(customer_id, team_name):
    ...
    # @app.route('/getResourceDetails/<int:customer_id>/<team_name>',methods=['GET'])
    @staticmethod
    def get_resource_details(customer_id, team_name):
    ...
    def getAllResourceNames():
        ...
        try:
            ...
            # records = list(MongoDBPersistence.application_analyst_mapping_tbl.find({},{"_id":0,"email_id":1,"resource_name":1,"resource_id":1}).sort('resource_name',1))
            # print(records)
            # print("json_util.dumps(records)", json_util.dumps(records))
            records = sorted(records, key=lambda i: (i['resource_name'].lower()))
        except Exception as e:
            return (e)
    ...
    def getMappedResourceNames():
        ...
        try:
            # records = list(MongoDBPersistence.resource_details_tbl.find({},{"_id":0,"email_id":1,"resource_name":1,"resource_id":1}).sort('resource_name',1))
    ...
    def updateApplication(currentMonthDays):
        ...
        # print("res id",mapping_doc["resource_id"])
        # MongoDBPersistence.resource_details_tbl.update_one({'email_id':resource_doc['email_id']},{'$set':resource_doc}, upsert=True)
        ...
        # new end
        # MongoDBPersistence.application_analyst_mapping_tbl.update_many({"resource_id":mapping_doc["resource_id"]}, {'$set':mapping_doc},upsert=True)
        # print('mapping_doc', mapping_doc)
        return "success"
    ...
    def getRosterScreenMappingDetails(customer_id, team_name, currentMonthDays, page_size, page_num):
        ...
        try:
            ...
            if (team_doc != None):
                dataset_id = team_doc['DatasetID']
            resource_email = list(
                MongoDBPersistence.roaster_tbl.find({"CustomerID": customer_id, "DatasetID": dataset_id},
                                                    {"_id": 0, "email_id": 1}))
            resource_email = list(set([r['email_id'] for r in resource_email]))
            print("Resource Email =   ", resource_email)

            skips = page_size * (page_num - 1)

            # result = list(MongoDBPersistence.roster_mapping_tbl.find({},{"_id":0}).sort('resource_name',1))
            result = list(
                MongoDBPersistence.roster_mapping_tbl.find({'email_id': {'$in': resource_email}}, {"_id": 0}).sort(
                    'resource_name', 1).skip(skips).limit(page_size))
            print("result_pagination---", list(set([r['email_id'] for r in result])))
            ...
            for record in result:
                ...
                if ('list_shift' in record.keys() and record["list_shift"] is not None and len(
                    ...
                    # print("new list shift..for email....",record["email_id"])
    ...
    def getRoasterMappingCount():
        ...
        roaster_mapping_count = MongoDBPersistence.roster_mapping_tbl.find().count()
        print("roaster_mapping_count", roaster_mapping_count)
        # if(roaster_mapping_count):
    ...
    def getRosterScreenMappingDetails1(currentMonthDays):
        ...
        try:
            ...
            # sorted_unique_resource=unique_resource.sort()
            for i in unique_resource:
                members_lst.append(i["_id"])

            # print("unique_resource@@@@@@@@@",unique_resource)
            # print("members_lst",members_lst)
            # result_shift_list=result["list_shift"]
            for record in result:
                date_form = parser.parse(record["start_date"])
                date_form1 = parser.parse(record["end_date"])
    ...
    def deletingExtraRows(day_tomorrow, curMonthDays, day, email_id, shift, year, month):
        try:
            ...
            if (int(day_tomorrow) <= curMonthDays):
                ...
                prv_month = str(month)
                month = int(month) + 1
                # print("day and day tomowo....",day, day_tomorrow,month,year)
                # print("prev month for day....",prv_month)
                if (len(str(day)) == 1):
                    day = '0' + str(day)
                if (len(str(day_tomorrow)) == 1):
                    day_tomorrow = '0' + str(day_tomorrow)
    ...
    def updateRosterMappingDetails(currentMonthDays):
        ...
        try:
            ...
            # roster_mapping = request.get_json()
            roster_mapping = request.get_json()["modifiedRosterDetails"]
            deleted_data = request.get_json()["deleteRosterList"]
            # print("delete roster data....",deleted_data)
            # print("roster mapping...",(roster_mapping))
            # print("update roster func....month days",currentMonthDays)
            # MongoDBPersistence.roster_mapping_tbl.delete_many({})
            # MongoDBPersistence.roster_mapping_tbl.insert_many(roster_mapping)
                ...
                if (
                    ...
                    ## if no change in start date and end date and shift is different from previous one.
                        ...
                        for date in k:
                            ...
                            day_tomorrow = str((day) + 1)
                            # print("date",date)
                            # print("day_tomorrow",day_tomorrow)
            ...
            if (result == "Success"):
                # print("roster update successfulllll")
                return "success"
            elif (result == "failure"):
                return "failure"
        except Exception as e:
    ...
    def insertDBShiftDetails(roster_data, currentMonthDays, deleteRosterData):
        ...
        try:
            #print("inside insertDBShiftDetails.......1......")
            #print("RosterData......",roster_data)
            #MongoDBPersistence.roaster_tbl.delete_many({})
            if(roster_data):
                with open('./config/shift_roaster_config.yaml') as roaster_file:
                    roaster_config = yaml.safe_load(roaster_file)
                ...
                for item in roster_data:
                    ...
                    if 'resource_group' in item.keys():
                        ...
                        # print("item[start_date] in insert db roster........",item["start_date"])
                        # print("item[end_date] .....",item["end_date"])

                        date_form = parser.parse(item["start_date"])
                        # print(date_form)
                        date_form1 = parser.parse(item["end_date"])
                        # print("weekoff.................................",item["weekoff"])
                        # weekoff changes - START
                        weekoffdates = item["weekoff"]
                        # weekoff changes - END
                        # print("Weekoffdates........................",weekoffdates)
                        ...
                        # print("check point 1")
                        # print("test.............",item["resource_group"])
                        for assign_grp in item["resource_group"]:
                            ...
                            while d0 <= d1:
                                ...
                                year = str(d0.year)
                                day = d0.day
                                # print('current roster day.......',month,year,day)
                                # print("check point 2")
                                curMonthDays = ResourceMasterData.numberOfDays(int(year), month)
                                # print("curMonthDays", curMonthDays)
                                document = {}
                                ...
                                day_tomorrow = str((day) + 1)
                                date = ''
                                # print("day ,day tomorrow ", day, day_tomorrow)
                                if (int(day_tomorrow) <= curMonthDays):
                                    if (len(str(day)) == 1):
                                        day = '0' + str(day)
                                    if (len(str(day_tomorrow)) == 1):
                                ...
                                d0 += delta
                            # print("start of weekend changes..................")
                            # weekoff changes - START
                            for date in weekoffdates:
                                # print("inside weekoff changes for loop.......",date)
                                # print("email id", item["email_id"])
                                email_id = item["email_id"]
                                date_form_weekoff = parser.parse(date)
                                # print("weekoff date............",date_form_weekoff)
                                ...
                                # print("......toBeDeleted_nightshift....",toBeDeleted_nightshift)
                                ...
                                # print(".........daybeforeDay",daybeforeDay, "datePrev_start_date", datePrev_start_date, "datePrev_end_date",datePrev_end_date)
                                ...
                                # print("..................prevdayHolidayShift", prevdayHolidayShift)
                                ...
                                if (len(toBeDeleted) > 0):
                                    ...
                                    for ele in toBeDeleted:
                                        id = ele["_id"]
                                        print(id)
                                        # roaster_tbl.delete_one({"_id":ObjectId(id)})
                                        MongoDBPersistence.roaster_tbl.delete_one({"_id": id})

            toBeDeletedRos = []
            # toBeDeletedHolidayRos = []
            # print("deleted data for roster.....",deleteRosterData)
            for record in deleteRosterData:
                ...
                # weekoff changes - END
                # print("Weekoffdates........................",weekoffdates1)
                d0 = date_form
                d1 = date_form1
                delta = timedelta(days=1)
                startIndex = date_form.day
                ...
                d0 = date_form
                d1 = date_form1
                # print("start date, end date for deletion......",d0,d1)
                while d0 <= d1:
                    month = d0.month
                    year = str(d0.year)
                    day = d0.day
                    ...
                    #print("before deleting....",start_date,end_date,shift,email_id)
