...
class CustomerMasterData(object):
    ...
    #Cleaning database TblCustomer & TblTraining : Keep only current(InProgress) & last (Approved)
    #Not used anywhere, for now written same code in saveFieldSelections API to clean table
    #def clean_TblCustomer(self, customer_id,status):
    #    o_fieldselection_list =  MongoDBPersistence.customer_tbl.find_one({'CustomerID':customer_id},{'FieldSelections':1, '_id':0})
    ...
    #     try:
    #         MongoDBPersistence.customer_tbl.update_one({'CustomerID':customer_id}, {"$set": {"FieldSelections":n_fieldselection_list}}, upsert=False)
    #         resp = 'success'
    #     except:
    #         resp = 'failure'
    #     return resp
    ...
    def getCustomerId(customer_name):
        ...
        customer = MongoDBPersistence.customer_tbl.find_one({"CustomerName": customer_name},{"CustomerID":1,"_id":0})
        if(customer):
            try:
            ...
            logging.info('%s: Failure: We are sorry, there is no customer with name %s registerd with us. please register with us before using ML algos.'%(RestService.timestamp(),customer_name))
            resp = 'failure'
        # print("Customer ID is ", temp['CustomerID'])
        return json_util.dumps(resp)
    
    @staticmethod
    def db_insert_ticket_weightage_details(customer_id,chosen_team):
        #print('In db_insert_application_details!!!!!!!!!!!!!');
        new_dataset_flag = 0
        file = request.files['ticketWeightageDetails']
        dataset_ = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"TeamName":chosen_team},{"DatasetID":1,"_id":0})
...
#            item.update( {"TrainingFlag":0})
    ...
    #    csv_df.drop_duplicates(subset ="number", keep = 'first', inplace = True)
        ...
        try:
            ...
            for ticket_weightage_doc in json_data:    
                #MongoDBPersistence.tickets_weightage_tbl.update_one({'ticket_type':ticket_weightage_doc['ticket_type'],'priority':ticket_weightage_doc['priority'],'status':ticket_weightage_doc['status']},{'$set':ticket_weightage_doc}, upsert=True)
                MongoDBPersistence.tickets_weightage_tbl.update_one({'ticket_type':ticket_weightage_doc['ticket_type'],'priority':ticket_weightage_doc['priority'],'status':ticket_weightage_doc['status'],'DatasetID':ticket_weightage_doc['DatasetID']},{'$set':ticket_weightage_doc}, upsert=True)
            ...
            if(new_dataset_flag):
                ...
                MongoDBPersistence.teams_tbl.update_one({'CustomerID':customer_id, "TeamName":chosen_team}, {"$set": {"DatasetID":dataset_id}}, upsert=False)
    ...
    def get_ticket_weightage_details(customer_id,chosen_team):
        ...
        try:
            team_doc=MongoDBPersistence.teams_tbl.find_one({'TeamName':chosen_team,'DatasetID':{'$exists':True}},{'_id':0,'DatasetID':1})
    ...
    def insert_ITSM_details():
        try:
            ...
            MongoDBPersistence.itsm_details_tbl.insert({"UserID": user_id, "Password":password, "ITSMToolName":itsm_toolname})
    ...
    def check_custom(field):
        try:
            ...
            logging.info("%s: Unable to connect to Config Files, check if custom fields are present or not"%RestService.timestamp())
            return ("failure", "failure")

    @staticmethod
    def get_icasUrl(field):
        try:
            config = configparser.ConfigParser()
            config["DEFAULT"]["path"] = "config/"
            config.read(config["DEFAULT"]["path"] + "Intent.ini")
            custom_flag = config["ICAS"][field]
            custom_module_url = config["ICAS"]["icasUrl"]
            return (custom_flag, custom_module_url)
        except:
            logging.info("%s: Unable to connect to Config Files, check if custom fields are present or not"%RestService.timestamp())
            return ("failure", "failure")
