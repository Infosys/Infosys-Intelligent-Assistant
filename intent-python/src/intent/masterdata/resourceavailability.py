...
class ResourceAvailabilityMasterData(object):
    ...
    #@app.route('/uploadRoasterDetails/<int:customer_id>/<team_name>', methods=["POST"])
    @staticmethod
    def db_insert_roaster_details( customer_id,team_name):
        start_time = datetime.now()
        logging.info('%s: In "db_insert_roaster_details()" method, Trying to get file from request object'%RestService.timestamp())
        ...
        if(dataset_):
            ...
            dataset_dict = MongoDBPersistence.datasets_tbl.find_one({"CustomerID" : customer_id, "DatasetID": {"$exists": True}},{'_id':0,"DatasetID":1,"DatasetName":1}, sort=[("DatasetID", -1)])
            if(dataset_dict):
        ...
        try:
            ...
            resource_lst=list(MongoDBPersistence.resource_details_tbl.find({'CustomerID':customer_id,'DatasetID':dataset_id},{'_id':0,'email_id':1}))
            app_lst=list(MongoDBPersistence.applicationDetails_tbl.find({'CustomerID':customer_id,'DatasetID':dataset_id},{'_id':0,'assignment_group':1}))
            ...
            if(len(resource_lst)!=0 or len(app_lst)!=0):
                ...
                for ws in wb.worksheets:
                    ...
                    if value == "Month":
                        ...
                        df = df[['CustomerID', 'DatasetID', 'assignment_group', 'email_id', 'application_name', 'shift', 'date', 'availability']]
                ...
                filtered_df = filtered_df.drop_duplicates(subset=['assignment_group', 'email_id', 'application_name', 'date'], keep='last').reset_index(drop=True)
                ...
                filtered_df = filtered_df[['CustomerID', 'DatasetID', 'assignment_group', 'email_id', 'application_name', 'shift', 'start_date',
                'end_date', 'availability', 'support_type']]
                df_records = filtered_df.to_dict('records')
                # MongoDBPersistence.roaster_tbl.delete_many({"CustomerID": 1, "DatasetID": 1})
                MongoDBPersistence.roaster_tbl.insert_many(df_records)
                ...
                #         if(email_id not in email_id_in_roaster):
                #             email_id_in_roaster.append(email_id)
                #         if(email_id in email_lst and assignment_group in assignment_lst):
                ...
                #                 shifts = list(pd.DataFrame(roaster_config).columns)
                ...
                #                         if(int(day_tomorrow)>no_of_days):
                #                             nxt_mon=mon+1
                #                             if(nxt_mon<=9):
                #                                 nxt_mon="0"+str(nxt_mon)
                                                                       
                #                             date_tomorrow=str(yer)+'-'+str(nxt_mon)+'-'+str('01')+' T'                                           
                ...
                logging.error('Could not get documents from either "resource_details_tbl" or from "applicationDetails_tbl" for cutomer id: %s, dataset id: %s'%(str(customer_id),str(dataset_id)))                
        except Exception as e:
        ...
        for i in  email_id_in_roaster:
            ...
            # print("roaster_assign_grp---------",roaster_assign_grp)
            logging.info(f"roaster_assign_grp: {roaster_assign_grp}")
            resource_id_name=list(MongoDBPersistence.resource_details_tbl.find({"email_id":i},{"_id":0,"resource_id":1,"resource_name":1,"email_id":1}))
            ...
            MongoDBPersistence.application_analyst_mapping_tbl.update_one({ "resource_id" :resource_id,"DatasetID":dataset_id},{"$set":{"resource_id" :resource_id,"email_id":i,
                            "resource_group":roaster_assign_grp,"resource_name":resource_name,"DatasetID":dataset_id}},upsert=True)
       
            #roaster_assign_grp
            # leave_list=holiday[i]
            ...
            for j in roaster_assign_grp:
                ...
                op=list(MongoDBPersistence.roaster_tbl.find({"email_id":email_id,"assignment_group":j},{"_id":0,"start_date":1,"end_date" :1,"shift":1}))
                # print("startdate_____________",op[0]["start_date"][0:4]+"/"+op[0]["start_date"][5:7]+"/"+op[0]["start_date"][8:10])
                # print("enddate_____________",op[-1]["start_date"][0:4]+"/"+op[-1]["start_date"][5:7]+"/"+op[-1]["start_date"][8:10])
                st_date=op[0]["start_date"][0:4]+"/"+op[0]["start_date"][5:7]+"/"+op[0]["start_date"][8:10]
                ed_date=op[-1]["start_date"][0:4]+"/"+op[-1]["start_date"][5:7]+"/"+op[-1]["start_date"][8:10]
                
                for o in op:
                    shift_name.append(o['shift'])
                #print("shift_names#########",shift_name)

                shifts=list(MongoDBPersistence.roaster_tbl.find({"email_id":email_id,"assignment_group":j},{"_id":0,"start_date":1}))
                #print("shifts",shifts)
                for date in shifts:
                ...
                #print("type of start date",type(startdate_value))
                ...
                #print("final shift list",shift_list)
                count=0
                ...
                for sh in range(0,len(shift_list)):
                    ...
                    # print("shift len",len(shift_list))
                    ...
                    if(sh==len(shift_list)-1 and shift_list[sh]!=""):
                        ...
                        # print("temp_shift last",temp_shift)
                        ...
                        if(temp_shift!=shift_list[sh-1]):
                            ...
                            if(int(month_value)==12):
                                formated_end_date=year_value+"/"+str(temp_end_date_for_last)+"/"+str(temp_end_date_for_last)
                            else:
                                formated_end_date=op[0]["start_date"][0:4]+"/"+str(next_month_value)+"/"+str(temp_end_date_for_last)
                            formated_st_date=op[0]["start_date"][0:4]+"/"+op[0]["start_date"][5:7]+"/"+str(temp_st_date)

                            repeat=list(MongoDBPersistence.roster_mapping_tbl.find({
                                     "resource_id" :resource_id,"email_id":i,"resource_id" :resource_id,"DatasetID":dataset_id,
                    ...
                    if(shift_list[sh]!=""):
                        ...
                        for sha in range(ind,len(shift_list)+1):    
                            ...
                            if(sha==len(shift_list) ):
                                ...
                                formated_end_date=op[0]["start_date"][0:4]+"/"+op[0]["start_date"][5:7]+"/"+str(temp_end_date)
                                if(temp_st_date<10):
                                ...
                                #for roast in roaster_assign_grp:
                                
                                #print("@@@@@@@@@@@email_id",i)
                                #print("@@@@@@@@@@@1 resource grp",roast)
                                repeat=list(MongoDBPersistence.roster_mapping_tbl.find({
                                     "resource_id" :resource_id,"email_id":i,"resource_id" :resource_id,"DatasetID":dataset_id,
                                     "start_date":formated_st_date,"end_date":formated_end_date,
                                     "list_shift":shift_list,"resource_group":roaster_assign_grp,
                            ...
                            #5.if current_shift != previousshift 
                                ...
                                formated_end_date=op[0]["start_date"][0:4]+"/"+op[0]["start_date"][5:7]+"/"+str(temp_end_date)
                                # print("formatted end date",formated_end_date)
                                ...
                                if len(repeat)==0:
                                    MongoDBPersistence.roster_mapping_tbl.insert_one({
                                     ...
                                     "resource_name":resource_name,"temp_resource_id" :resource_id,
                                     "resource_shift":temp_shift})    
                                     #"weekoff":holiday[i] removed in above query
                ...
                resource_roastermap=list(MongoDBPersistence.roster_mapping_tbl.find({"email_id":i,"resource_group":j},
                    {"email_id":1,"start_date":1,"end_date":1}))
                # print("resource_roastermap",resource_roastermap)
                ###change
                if(holiday):
                    if i not in holiday.keys():
                        holiday[i]=[]
                    holiday_of_resource=holiday[i]
                    # print("holiday_of_resource",holiday_of_resource)
                    for hol in range(len(resource_roastermap)):
                        ...
                        for h_res in holiday_of_resource:
                            #if(int(h_res[8:]) > hol_end_date and int(h_res[8:])<hol_next_start_date and h_res not in new_hol_list):
                            if(int(h_res[8:])>= hol_start_date and int(h_res[8:])<=hol_end_date and h_res not in new_hol_list):
                                new_hol_list.append(h_res)
                        # print("new_hol_list",new_hol_list)
        ...
        return json_util.dumps(ignored_lst)
    
    
    #@app.route('/getShiftDetails/<resource_name>',methods=['GET'])
    @staticmethod
    def get_roaster_details_of_resource(customer_id,team_name,email_id):
        try:
    ...
    #@app.route('/updateRoaster/<resource_name>/<start_date>/<end_date>',methods=['PUT'])
    @staticmethod
    def update_roaster(customer_id,team_name,email_id,start_date,end_date):
    ...
    #    --temp variables--
        ...
        print("date1...............",date1)
        # date_lst=start_date.split(' ')
        # month= a.get(date_lst[1])
        day = date1.day
        month = date1.month
        if(len(str(day))==1):
            day='0'+str(day)
        ...
        try:
            ...
            if(team_doc!=None):
                ...
                resourcedetails = list(MongoDBPersistence.resource_details_tbl.find({"email_id":email_id},{"_id":0,"resource_name":1,"resource_id":1}))
                ...
                },{'$set':{"resource_shift":"L","weekoff":[],"list_shift":[],"email_id":email_id,'start_date':mappingstart_date,
    ...
    #@app.route('/getRoasterDetails/<int:customer_id>/<team_name>',methods=['GET'])
    @staticmethod
    def get_roaster_details( customer_id,team_name):
        ...
        try:
            if(today==None):
                ...
                today=str(todays_date)+' T'+str(format(time.hour,'02d'))+':'+str(time.minute)+':'+str(time.second)
            logging.info('%s: formated date is: %s, Trying to get document from "teams_tbl"'%(RestService.timestamp(),today))
            team_doc=MongoDBPersistence.teams_tbl.find_one({'TeamName':team_name,'DatasetID':{'$exists':True}},{'_id':0,'DatasetID':1})
            ...
            if(team_doc!=None):
                ...
                # roster_mapping_list=[]
                # roster_mapping_list=list(MongoDBPersistence.roster_mapping_tbl.find({"DatasetID":dataset_id},{'_id':0}).sort('resource_name',1))
                ...
                if(len(roaster_lst)!=0):
                    ...
                    if(roaster_lst):
                        ...
                        roaster_lst=MongoDBPersistence.roaster_tbl.find_one({'start_date':{'$gte':today}})
                        logging.error('%s:  No further data for the current month or No data for the current month in "roaster_tbl"'%RestService.timestamp())
                        if(not roaster_lst):
                            logging.error('%s: Could not get shift details for future!'%RestService.timestamp())
                            return json_util.dumps('failure-02')
                        else:
                            logging.error('%s: Could not get shift details for further days of the month: %s from "roaster_tbl"!'%(RestService.timestamp(), str(todays_date.month)))
