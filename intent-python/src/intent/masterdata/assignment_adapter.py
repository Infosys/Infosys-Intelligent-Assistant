...
class WorkloadAssignment():
    ...
    def __init__(self,possible_assignee=None,possible_assignee_for_assignment=None):
        ...
        # if('roundrobin_enabled' in keys and doc['roundrobin_enabled'] == 'true'):
        #     WorkloadAssignment.round_robin_flag = True


    def find_possibleassignees(self):
        ()
        logging.info('%s: In "find_possibleassignees" method, argument - assign grp: %s'%(RestService.timestamp(), self.possible_assignee))
        customer_id = 1
        ...
        app_lst=list(MongoDBPersistence.applicationDetails_tbl.find({'assignment_group':self.possible_assignee}, {"_id":0}))        
        ...
        if round_robin_flag == True:
            ...
            # print("resources list in find_possible assignees ",resource_lst)
            if(resource_lst):
                for resource_doc in resource_lst:
                    incident_lst= list(MongoDBPersistence.rt_tickets_tbl.find({"assigned_to":resource_doc['resource_name']},{'number':1,"_id":0}))
            ...
            # print("After Round robin list :",list(data_sorted.keys()))
            ...
            if((app_weightage >= 0) and roaster_lst):
                ...
                logging.error('Could not found value Either from "getAppWeightage()" or from "getRoasterList()" for assign group: %s'%(self.possible_assignee))
    ...
    def find_roundrobin_assignees(assignment_group,incidentCount):
        ()
        logging.info('%s: in "def find_roundrobin_assignees" method, assign grp: %s, incident count: %s'%(RestService.timestamp(), assignment_group,incidentCount))
        try:
            resource_name_lst = []
            roaster_lst = []
            resource_lst = []
            ...
            if(roaster_lst):
                ...
                if(resource_lst):
                    for resource_doc in resource_lst:
                        tickets_assigned=0
                        workload = 0.0
                        incident_lst= list(MongoDBPersistence.rt_tickets_tbl.find({"assigned_to":resource_doc['resource_name']},{"_id":0,'number':1}))
                        name_ticketcount[resource_doc['resource_name']] = len(incident_lst)
                    data_sorted = {k: v for k, v in sorted(name_ticketcount.items(), key=lambda x: x[1])}
    ...
    def find_assignees(assignment_group, incidentCount, incident_tckts):
        ...
        try:
            ...
            try:
                ...
                logging.error('priorityField is not defined in "config/intent.ini" file. Define it for Assignee Prediction')
...
#                        workload= 0.0
                    ...
                    while i < incidentCount and assigned: 
                        ...
                        incident_doc = MongoDBPersistence.rt_tickets_tbl.find_one({'number':incident_no},{'_id':0, 'state':1, priority_field:1})
                        priority = incident_doc[priority_field].split()[2]
                        logging.info(f"priority: {priority}")
                        if priority == 'Moderate':
                            priority = 'Medium'
                        logging.info(f"priority: {priority}")
                        tckt_weightage_doc = MongoDBPersistence.tickets_weightage_tbl.find_one({'priority':priority, 'status':'InProgress'},{'_id':0,'ticket_weightage':1})
    ...
    def find_assigneesForAssignmentGroups(self):
        ...
        try:
            ...
            for ag in counts.keys():
                ...
                #res = WorkloadAssignment.find_assignees(ag,counts[ag])
                # round_robin_flag=True
                # if WorkloadAssignment.round_robin_flag == True:
                round_robin_flag = False
                doc=MongoDBPersistence.assign_enable_tbl.find_one({})
                if(doc['roundrobin_enabled'] == 'true'):
                    round_robin_flag = True
                if round_robin_flag == True:
                    # print("assignementgroups and count ",ag,counts[ag])
                ...
                if(res):
                    ...
                    for k in range(0 , len(self.possible_assignee_for_assignment)):
                        ...
                        # if self.possible_assignee_for_assignment[k] == ag:
...
class OneByOneMappingAssignment():
    ...
    def manualassignee(self):
        assignee_list = MongoDBPersistence.manual_assignee_tbl.find_one({"CustomerID":self.customer_id, "DatasetID": self.dataset_id}, {"_id": 0, "MappingFields": 1})
