...
class TeamMasterData(object):
    ...
    #@app.route('/getTeamID/<int:customer_id>/<team_name>', methods=["GET"])
    @staticmethod
    def getTeamID(customer_id, team_name):

        team_ = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"TeamName":team_name},{"TeamID":1,"_id":0})
        if(team_):
    ...
    def getTeamName(customer_id, dataset_id):
        ...
        team_ = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"DatasetID":dataset_id},{"TeamName":1,"_id":0})
        if(team_):
    ...
    #@app.route('/getTeams/<int:customer_id>', methods=["GET"])
    @staticmethod
    def getTeams( customer_id):
        teams_dict = {}
        teams_list = []
    ...
    #@app.route('/getDatasetTeamNames/<int:customer_id>', methods=["GET"])
    @staticmethod
    def getDatasetTeamNames(customer_id):
        teams_dict = {}
        teams_list = []
    ...
    @staticmethod
    def getAllOpenIncidentCount(customer_id):
        count = MongoDBPersistence.rt_tickets_tbl.find({"CustomerID" : customer_id, "state" : "New", "DatasetID": {"$exists": True}}).count()
        if(count):
    ...
    def getOpenIncidentCountTeam(customer_id, team_name):
        ...
        dataset_id_dict = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"TeamName":team_name},{"DatasetID":1,"_id":0})
        if(dataset_id_dict):
            dataset_id = dataset_id_dict['DatasetID']
            count = MongoDBPersistence.rt_tickets_tbl.find({"CustomerID" : customer_id, "state" : "New", "DatasetID": dataset_id}).count()
            if(count):
                resp = str(count)
            else:
                resp = 'empty'
    ...
    #@app.route('/getNoDatasetTeamNames/<int:customer_id>', methods=["GET"])
    @staticmethod
    def getNoDatasetTeamNames(customer_id):
        teams_dict = {}
        teams_list = []
        for team in list(MongoDBPersistence.teams_tbl.find({"CustomerID" : customer_id, "DatasetID": {"$exists": False}},{'_id':0,"TeamName":1})):
            teams_list.append(team['TeamName'])
        teams_dict['Teams'] = teams_list
        return json_util.dumps(teams_dict)
    ...
    def addTeam(customer_id):
        ...
        for team in team_list:
            ...
            if('TeamID' not in keys and team['TeamName'] not in team_names):
                ...
                MongoDBPersistence.teams_tbl.update_one({'TeamID':team['TeamID'],'CustomerID':customer_id},{'$set':team})
    ...
    def removeTeam(customer_id,team_list):
        ...
        if(team_list):
            ...
            for team_id_str in team_string_lst:
                ...
                logging.info('%s: trying to update team documents to team collection'%RestService.timestamp)        
                MongoDBPersistence.teams_tbl.delete_one({'CustomerID':customer_id,'TeamID':team_id,'DatasetID':{'$exists':False}})
                logging.info('%s: documents updated successfully'%RestService.timestamp) 
            return 'success'
        else:
    ...
    def getTagDatasetTeamNames(customer_id):
        ...
        teams_list = []
        datasetid_list = []
        for team in list(MongoDBPersistence.teams_tbl.find({"CustomerID" : customer_id, "TagsDatasetID": {"$exists": True}},{'_id':0,"TeamName":1,"TagsDatasetID":1})):
            teams_list.append(team['TeamName'])
    ...
    def getNoTagDatasetTeamNames(customer_id):
        ...
        datasets_list = []
        tags_team_list = []
        for team in list(MongoDBPersistence.teams_tbl.find({"CustomerID" : customer_id, "TagsDatasetID": {"$exists": False}},{'_id':0,"TeamName":1})):
            teams_list.append(team['TeamName'])
        for dataset_team in list(MongoDBPersistence.training_hist_tbl.find({"CustomerID" : customer_id, "TrainingStatus": {"$exists": True,"$in": ['Approved']}},{'_id':0,"DatasetID":1})):
            datasets_list.append(dataset_team['DatasetID'])
        for dataset in datasets_list:
            trained_teams_list.append(MongoDBPersistence.teams_tbl.find_one({"CustomerID":customer_id,"DatasetID":dataset},{"_id":0,"TeamName":1})['TeamName'])
