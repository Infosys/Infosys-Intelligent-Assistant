...
from intent.utils.utils import Utils
import configparser
import pickle
import matplotlib
import datetime
...
# @app.route('/api/generate_heat_map', methods=['GET'])
# def generate_heat_map():
#     return PMA.generate_heat_map()
...
class PMA(object):
    ...
    def generatePMAGraph(customer_id, dataset_id, year,month, selected_field):
        ...
        #print(year, month, selected_field)
        #logging.info(year, month, selected_field)
    ...
    # @staticmethod
    # def generatePMAGraph_ORIGINAL(customer_id, dataset_id, year,month, selected_field):
    #     print("inside pma Bar graph...")
    #     print(year, month, selected_field)
    #     ()
    #     PMA_data = list(MongoDBPersistence.cluster_table.find({'CustomerID': 1, "DatasetID": 1}))
    #     pd_data = pd.DataFrame(PMA_data)
    #     logging.info(pd_data.columns)

    #     pd_data['created'] = pd.to_datetime(pd_data.loc[:, 'created'], errors='coerce')

    #     start_date = (pd_data['created']).min()
    #     end_month = (pd_data['created']).max() + np.timedelta64(1, 'M')
    #     end_year = (pd_data['created']).max() + np.timedelta64(1, 'Y')

    #     month_list = [str(i.strftime("%B-%Y"))+(":" + str(selected_field))
    #                   for i in pd.date_range(start=start_date, end=end_month, freq='M')]

    #     year_list = [str(i.strftime("%Y"))+(":" + str(selected_field))
    #                  for i in pd.date_range(start=start_date, end=end_year, freq='Y')]

    #     print("year list..............",year_list)
        
        
    #     path_check = os.path.join('static/assets/video/PMA_'+year+'_'+month+'_'+selected_field+'.png')
    #     print('save_path', path_check)

    #     try:
    #         vocab_path = os.path.join('static/assets/video/PMA_'+year+'_'+month+'_'+selected_field+'.png')

    #         if year != '' and month=='undefined':
    #             ani = PMA.plot_graph_IIA_Year(year,pd_data,selected_field)

    #         elif year != '' and month != '':
    #             ani = PMA.plot_graph_IIA_month(year,month,pd_data, selected_field)

    #             bar_fig.savefig(vocab_path, dpi=500)
    ...
    # @staticmethod
    # def plot_graph_IIA_month(year,month,pd_data, selected_field ):
    #     print('inside PMA Bar month graph...')
    ...
    #         months_label_list.clear()

    #     months_label_list.append(_month + str(year1))
    #     print('monthlabellist', months_label_list)

    #     try:
    #         ()
    #         bar_ax.clear()

    #         filter_year = ((pd_data['created'] >= str(year1)) & (
    #                 pd_data['created'] < str(year1 + 1)))
    #         year_data = pd_data[filter_year]

    #         filter_month = (year_data['created'].dt.month >= month) & (
    #                 year_data['created'].dt.month < (month + 1))
    #         month_data = year_data[filter_month]

    #         Assignment_group_counts = Counter()
    #         for j in month_data[selected_field]:
    #             Assignment_group_counts.update(str(j).split('\n'))

    #         assignment_grp_name_list = []
    #         assignment_grp_count_list = []

    #         number_of_rows_display = 12

    #         for item in Assignment_group_counts.most_common(number_of_rows_display):
    #             assignment_grp_name_list.append(item[0])
    #             assignment_grp_count_list.append((item[1]))

    #         assignment_grp_name_list.reverse()
    #         assignment_grp_count_list.reverse()
    #         bar_ax.xaxis.set_ticks_position('top')
    #         for i, (count, name) in enumerate(zip(assignment_grp_count_list, assignment_grp_name_list)):
    #             bar_ax.text(count, i, " {}".format(count), va='center',
    #                         ha='left', size=30, fontstyle='italic', weight=400, label=assignment_grp_name_list)
    #             bar_ax.barh(name, count, label=name)
    #         print("length of lst",assignment_grp_name_list)
    #         # colors_list = ['#adb0ff', '#ffb3ff', '#90d595', '#e48381','#aafbff', '#f7bb5f', '#eafb50']

    #         # bar_ax.barh(assignment_grp_name_list, assignment_grp_count_list,
    #         #             color=[x for x in colors_list])
    #         bar_ax.text(0.5, 0, _month + " " + str(year1),
    #                     transform=bar_ax.transAxes, size=60, ha='right', color='red')

    #         bar_ax.tick_params(axis="x", labelsize=35)
    #         bar_ax.tick_params(axis="y", labelsize=0)
    #         # bar_ax.legend(title='Number of Incidents', labels=assignment_grp_name_list, loc=2, borderaxespad=0, fontsize=15, frameon=False)
    #         bar_ax.spines['top'].set_visible(False)
    #         bar_ax.spines['right'].set_visible(False)
    #         bar_ax.spines['left'].set_visible(False)
    #         bar_ax.spines['bottom'].set_visible(False)
    #         bar_ax.set_facecolor("#e8e8e8")

    #         plt.box(False)
    #         bar_ax.legend(fontsize=30)
    #     except Exception as ex:
    #         print('Exception...', ex)


    # @staticmethod
    # def plot_graph_IIA_Year(year,pd_data,selected_field):
        
    #     print('inside PMA bar year...')
    #     _year = year
    #     print("year", _year)
    ...
    #         filter_year = (pd_data['created'] >= str(year1)) & (pd_data['created'] < str(year1 + 1))
            
    #         plt.cla()
            
    #         data_for_input_year = pd_data[filter_year]
            
    #         print("data for input year", data_for_input_year )

    #         Assignment_group_counts = Counter()

    #         for j in data_for_input_year[selected_field]:
    #             Assignment_group_counts.update(str(j).split('\n'))
    ...
    #         print("xaxis; ",assignment_grp_name_list)
    #         print("yaxis; ",assignment_grp_count_list)

    #         # colors_list = ['#adb0ff', '#ffb3ff', '#90d595', '#e48381','#aafbff', '#f7bb5f', '#eafb50']

    #         for i, (count, name) in enumerate(zip(assignment_grp_count_list, assignment_grp_name_list)):
    #             bar_ax.text(count, i, " {}".format(count), va='center',
    #                     ha='left', size=30, fontstyle='italic', weight=400)
    #             bar_ax.barh(name, count, label=name)
    ...
    #         bar_ax.xaxis.set_ticks_position('top')
    #         bar_ax.text(0.5, 0, year1, transform=bar_ax.transAxes,
    #                 size=60, ha='right', color='red')
    #         bar_ax.tick_params(axis="x", labelsize=35)
    #         bar_ax.tick_params(axis="y", labelsize=0)
    #         bar_ax.spines['top'].set_visible(False)
    #         bar_ax.spines['right'].set_visible(False)
    ...
    #         path_check = os.path.join('static/assets/video/PMA_'+year+'_'+'undefined'+'_'+selected_field+'.png')
    #         print('save_path',path_check)

    #         bar_fig.savefig(path_check, dpi=300)
    ...
    def monthlyTrend_PMA(customer_id,dataset_id,from_year,to_year):
        ...
        for i in range(len(filtered_data)):
            #print(cre_list[0]['created'])
        #  my_string=(cre_list[i]['created'].split(" "))[0]
    ...
    @staticmethod
    def displayPieGraph_PMA(customer_id,dataset_id,selected_field,year,month):
        #print("Control is here PMA1")
        # select_field = list(MongoDBPersistence.training_tickets_tbl.find({"CustomerID": customer_id, "DatasetID":dataset_id},{selected_field:1,"_id":0}))

        # temp_list=[]
        # for i in range(0,len(select_field)-1):
        #     temp_list.append(select_field[i][selected_field])
            
        #     from collections import Counter
        #     select_field_dict = dict(Counter(temp_list))
        # print(len(temp_list))
        # print("data for empty ",select_field_dict)
        # for key,value in select_field_dict.items():
        #     if(key==""):
        #         select_field_dict["Unassigned"]=select_field_dict.pop(key)
        #         select_field_dict["Unassigned"]=value
        
        # new_select_field_dict=dict(sorted(select_field_dict.items(), key=lambda kv: kv[1], reverse=True))#sorting in descending order
        # if(len(new_select_field_dict)>15):
        #   new_select_field_dict=Training.take(15, new_select_field_dict.items())#taking only top 15 
        select_field = list(MongoDBPersistence.cluster_table.find({"CustomerID": 1, "DatasetID":1}))

        # mydata1 = list(MongoDBPersistence.nbn_tbl.find({'CustomerID': 1, "DatasetID": 1}))
        # print(select_field)
        ...
        if(select_field):
            ...
            #logging.info('%s: Generated json based on Predict field...'%RestService.timestamp())
        else:
            resp = ''
            #logging.info('%s: Predict field not found in TblIncidentRT , returning empty string...'%RestService.timestamp())
    ...
    def displayPieWordCloud_PMA(customer_id, dataset_id, selectedfield,field_value,unassigned_value,number_of_words,cloud_type):
        ...
        if(unassigned_value=="true"):    
            # return json_util.dumps({'one':'arun','two':'hello'})
            ...
            input_df = pd.DataFrame(list(MongoDBPersistence.cluster_table.find({'CustomerID': 1, "DatasetID": 1,selectedfield:"" },{'_id':0,"description":1})))
            ...
            for sentence in sentences:
                ...
                #ps = PorterStemmer()
                #clean_sentence = [ps.stem(word) for word in clean_sentence if not word in set(final_stopwords)]
            ...
            input_df = pd.DataFrame()
            input_df = pd.DataFrame(list(MongoDBPersistence.cluster_table.find({'CustomerID': 1, "DatasetID": 1,selectedfield:field_value },{'_id':0,"description":1})))
    ...
    def generate_heatmap(input_year, selected_group):
        ...
        try:         
            ...
            #new_data = pd.read_csv(r'C:\Users\ankit.singh17\Downloads\Python\DataSheets\hershey_new.csv', encoding = 'latin')
    ...
    def generatePMALineGraph(data):
        ...
        for slt_grp_lst in selected_group:
            ...
            #plt.plot(month_list_x, count_list_y, label=i, marker='o')
        ...
        plt.gcf()
        #plt.setp(line_ax.get_xticklabels(), rotation = 30)
        #plt.setp(line_ax.get_xticklabels(), rotation = 30)
    ...
    # @staticmethod
    # def WeightedMovingAverage_Original():
    ...
    #                 #plt.figure(figsize=(10, 5))
    #                 plt.grid(True, lw=2)
    #                 plt.plot(x, y1, lw=5, label='count for '+group)
    #                 plt.plot(x, y2, lw=5, label='count_mean for '+group)
    #                 plt.ylabel("count_mean + count", fontsize=40)
    #                 plt.xlabel("no.of days", fontsize=40)
    #                 plt.xticks(fontsize=40)
    #                 plt.yticks(fontsize=40)
    #                 plt.title("Weighted Moving Average", fontsize=50)
    #                 plt.legend(loc="upper right", prop={"size": 40})

    #             fig = plt.gcf()
    #             fig.set_size_inches(50, 30)
    #             fig.savefig(path_check, dpi=400)
    ...
    #                 #plt.figure(figsize=(10, 5))
    #                 plt.grid(True, lw=2)
    #                 plt.plot(new_data2['Count_mean'],label='count mean for ' + group, lw=6)
    #                 plt.ylabel("count_mean", fontsize=50)
    #                 plt.xlabel("no.of days", fontsize=50)
    #                 plt.xticks(fontsize=40)
    #                 plt.yticks(fontsize=40)
    #                 plt.title("Weighted Moving Average", fontsize=50)
    #                 plt.legend(loc="upper right", prop={"size": 40})
    ...
    #             #weighted_mov_avg_path = os.path.join('static/assets/video/' + field_value + s + '_weighted-moving-average.png')
    #             fig.savefig(path_check, dpi=400)
    #             print('done...')
    ...
    #     except Exception as ex:
    #         print("exception inside weightegd moving avg....", ex)
    #         return "error"
    ...
    # fig,axes=plt.subplot(figsize=(16,5),dpi=300)
        if(group!='All'):        
        ...
        try:
            ...
            if os.path.exists(dir_name1) and os.path.exists(dir_name2):
                ...
                try:
                    ...
                    #  df_ = pd.read_csv(r"C:\Users\manjunath.magdum\Downloads\train_5_20_2021.csv",encoding='ISO-8859-1')
    ...
    def db_insert_clustering_tickets(customer_id,dataset_id):
        ...
        # dataset_ = teams_tbl.find_one({"CustomerID": customer_id},{"DatasetID":1,"_id":0})
        # if(dataset_):
        #     #Dataset exist for the team
        #     logging.info('%s: Getting old dataset details.'%timestamp)
        ...
        # dataset_ = MongoDBPersistence.datasets_tbl.find_one({"CustomerID" : 1, "DatasetID": {"$exists": True}},{'_id':0,"DatasetID":1}, sort=[("DatasetID", -1)])
        ...
        if(dataset_):
            ...
            #Newly adding the dataset
            # logging.info('%s: Adding new dataset.'%timestamp)
            #getting max dataset id for the customer, so that new dataset id = old + 1
            # dataset_dict = MongoDBPersistence.datasets_tbl.find_one({"CustomerID" : 1, "DatasetID": {"$exists": True}},{'_id':0,"DatasetID":1}, sort=[("DatasetID", -1)])
        ...
        if('number' not in csv_df.columns):
            logging.info('%s: Please rename ticket_id/Incident_id column to "number".'%timestamp)
            return 'failure'
        ...
        if(training_row):
            ...
            if(not set(csv_df_cols) == set(prev_uploaded_cols)):
                # logging.info('%s: Column names of previously uploaded data dont mach with uploaded csv.'%timestamp)
                return 'failure'
    #=======
        ...
        # if(trainign_row):
            ...
            # if(not set(csv_df_cols) == set(prev_uploaded_cols)):
            #     logging.info('%s: Column names of previously uploaded data dont match with uploaded csv.'%timestamp)
            #     return 'failure'
    #>>>>>>> b88b9156b4588d2fe9a76307a8ac287c82e5f830
        ...
        try:
            ...
            if(new_dataset_flag):
                # logging.info('%s: Trying to insert new dataset into TblDataset...'%timestamp)
                MongoDBPersistence.datasets_tbl.insert_one(new_dataset_dict)
                # logging.info('%s: Trying to update TblTeams with Dataset details...'%timestamp)
                MongoDBPersistence.teams_tbl.update_one({'CustomerID':customer_id}, {"$set": {"DatasetID":dataset_id}}, upsert=False)
            # logging.info('%s: Records inserted successfully.'%timestamp)
            ...
            # logging.error('%s: Error occurred: %s '%(timestamp,str(e)))
            # logging.error('%s: Possible error: Data format in csv not matching with database constarints.(unique key & not null)'%timestamp)
            resp = 'failure'
        return resp
    ...
    def getDatasetDetails1(customer_id):
        ...
        if(dataset_dict):
            ...
            # logging.info('%s: Dataset details not found for the team.'%timestamp) 
