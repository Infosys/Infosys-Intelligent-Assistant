...
#from intent.RelatedSearch.word2vecsearch import RelatedSearch
...
# @app.route("/api/getRelatedTickets/<int:customer_id>/<int:dataset_id>/<incident_number>", methods=['GET'])
# def getRelatedTickets(customer_id, dataset_id, incident_number):
#     return RelatedSearch.getRelatedTickets(customer_id, dataset_id, incident_number)
...
class RelatedSearch:
    ...
    def concatinateFields(customer_id, dataset_id, type_of_tickets, incident_number=0):
        ...
        try:
            ...
            logging.error('%s: Error occured in Concatinating input fields in vectorSearchModel: %s' % (RestService.timestamp(), str(e)))
            return 'failure'
            raise
        input_df['in_field'] = ''
    ...
    def normalize_description(description):
        try:
            ...
            raise
        #print("normalize desc",type(descr_english))
        return ""
    ...
    def get_raw_text(customer_id, dataset_id, field_name, incident_number, type_of_tickets='closed'):
        ...
        try:
            ...
            logging.info(" %s Key not found in Tbl Incident Training while working with get_raw_text in vectorSearchmodel- %s"  % (RestService.timestamp(), str(e)))
            raise
    ...
    def getRelatedTickets(customer_id, dataset_id, incident_number,search_alogorithm):
        ...
        try:   
                    ...
                    if rel_weight > match_percentage:
                        ...
                        for field_name in display_field_list:
                            related_ticket[field_name] = RelatedSearch.get_raw_text(customer_id, dataset_id, field_name,
                                                                                        ticket_number)
                        # related_ticket['number']= related_ticket.pop(ticket_id_field_name)
                        # related_ticket['description']= related_ticket.pop(description_field_name)
                        # related_ticket['assignment_group']= related_ticket.pop(group_field_name)
                        # related_ticket['close_notes']= related_ticket.pop(close_notes_field_name)
            ...
            logging.info("%s exception occured in vector_getRelatedTickets Method for a Incident -%s with exception details as - %s" % (RestService.timestamp(),incident_number, str(e)))
            return result
            raise
        ...
        try:
            ...
            if (search_alogorithm == 'BERT'):
                ...
                for i in range(5):
                    ...
                    metrics = pairwise_similarities[0][tick_row]
                    Normalized_Description = df.iloc[tick_row]['Normalized_Description']
                    suggestion = {"number": ticket_num,"sims": float(metrics), "closed_tickets":in_field, "Normalized_Description":Normalized_Description}
                    ticket_details.append(suggestion)
                similarity_df = pd.DataFrame(ticket_details)
            else:
                if(search_alogorithm=="word2Vec"):
            ...
            logging.info("%s exception occured in vector_getRelatedOpenTickets Method for a Incident -%s with exception details as - %s" % (RestService.timestamp(),incident_number, str(e)))
            raise 