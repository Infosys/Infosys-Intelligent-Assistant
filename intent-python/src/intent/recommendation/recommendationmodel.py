...
from intent.recommendation.textRankSearch import TextRankSearch
from intent.recommendation.word2vecsearch import RelatedSearch
from intent.recommendation.KASearch import KAreleatedsearch
...
@app.route("/api/getRelatedTickets/<int:customer_id>/<int:dataset_id>/<incident_number>/<algorithm_name>", methods=['GET'])
...
@app.route("/api/display_resolution/<int:customer_id>/<int:dataset_id>/<incident_number>/<tab_name>/<cloud_type>/<algorithm_name>",
           methods=['GET'])
def display_resolution(customer_id, dataset_id, incident_number, tab_name, cloud_type,algorithm_name):
    return Recommendation.display_resolution(customer_id, dataset_id, incident_number, tab_name, cloud_type,algorithm_name)


# @app.route("/api/getKnowledgeArticlesSummary/<int:customer_id>/<int:dataset_id>/<incident_number>", methods=['GET'])
# def getKnowledgeArticlesSummary(customer_id, dataset_id, incident_number):
#     return Recommendation.getKnowledgeArticlesSummary(customer_id, dataset_id, incident_number)

@app.route("/api/getRelatedKnowledgeArticles/<int:customer_id>/<int:dataset_id>/<incident_number>/<algorithm_name>", methods=['GET'])
...
# @app.route("/api/getRelatedKnowledgeSummary/<int:customer_id>/<int:dataset_id>/<incident_number>", methods=['GET'])
# def getRelatedKnowledgeSummary(customer_id, dataset_id, incident_number):
#     return Recommendation.getRelatedKnowledgeSummary(customer_id, dataset_id, incident_number)
...
class Recommendation:
                ...
                if (incident_number == 0):  # Generate TF for all corpus
                    ...
                    input_df = pd.DataFrame(list(MongoDBPersistence.rt_tickets_tbl.find(
                        ...
                        {ticket_id_field_name: 1, '_id': 0,
                         input_field_list[0]: 1})))  # ,pred_field:{"$exists":True,"$ne": ""}
                    for input_field_ in input_field_list[1:]:
    ...
    def generate_TF(customer_id, dataset_id, type_of_tickets='closed'):
        ...
        if (field_mapping['Source_ITSM_Name'] == 'SNOW'):
            ...
            close_notes_field_name = 'close_notes'
        # incident_list = list(MongoDBPersistence.training_tickets_tbl.find({"CustomerID": customer_id, "DatasetID": dataset_id}, {'_id': 0}))
        incident_list = Recommendation.concatinateFields(customer_id, dataset_id, type_of_tickets)
        # corpus = 'description'
        # word_doc_dict: dictionary with word as key and a dictionary with documents
        # where it occurs as keys and term frequency(tf) values as values'
        word_doc_dict = {}
    ...
    def getRelatedTickets_(customer_id, dataset_id, incident_number):
        ...
        result_tfidf_dict = {}
        # query = MongoDBPersistence.rt_tickets_tbl.find_one({"CustomerID": customer_id, "DatasetID": dataset_id, 'number': incident_number}, {'_id': 0})
        ()
        ...
        if (len(result_tfidf_indices) == 0):
            ...
            # if len(result_tfidf_indices) > 10:
            #    print("Returning 1st of 10 results")
            ...
            for (index, tup) in enumerate(result_tfidf_indices):
                ...
                # print("%s::%s -> %s" % (tup[0], tup[1], Recommendation.get_raw_text(1, 1, 'description', tup[0])))
                # print("\n")
            # print(result_list)
            # print(tup[0])
    ...
    def getRelatedTickets(customer_id, dataset_id, incident_number, algorithm_name):
        ...
        relatedClosedTickets= list(MongoDBPersistence.related_tickets_tbl.find({"RT_Ticket_Number":incident_number,"Algorithm_name":algorithm_name},{'Related_ticket_info':1,'_id':0}))
        #print("relatedClosedTickets",relatedClosedTickets)
        # for each in relatedclosedTickets:
        #     print(relatedclosedTickets[0]['Related_ticket_info'])
            # print(type((relatedclosedTickets[0]['Related_ticket_info'])))
        # result_list=[]
        # for each in relatedIncidentsList:
        #     exam=list(MongoDBPersistence.training_tickets_tbl.find({"number":each},{'_id':0,'number':1,'description':1,'close_notes':1}))
        ...
        if  len(relatedClosedTickets) :            
            ...
            for each in relatedIncidentsList:
                exam = list(MongoDBPersistence.training_tickets_tbl.find({ticket_id_field_name: each}, dict_select_fields))
                dict_incident_fields = {}
                for field in display_field_list:
                    dict_incident_fields[field] = exam[0][field]
                #result_list.append({'description':exam[0]['description'],'close_notes':exam[0]['close_notes'],'number':exam[0]['number']})
            ...
            try:
                ...
                logging.info("%s exception occured in getRelatedTickets Method for a Incident -%s with exception details as - %s" % (RestService.timestamp(),incident_number, str(e)))
                raise 
            ...
            if len(related_results) > 0:
                ...
                #print("time closedtic",stop001-start01)
            ...
            if(search_alogorithm=="word2Vec" or search_alogorithm =="doc2Vec" or search_alogorithm =="BERT" ):
                related_results=RelatedSearch.getRelatedTickets(customer_id, dataset_id, incident_number,search_alogorithm)
    ...
    def getRelatedOpenTickets(customer_id, dataset_id, incident_number):
        ...
        relatedOpenTickets= list(MongoDBPersistence.related_tickets_tbl.find({"RT_Ticket_Number":incident_number},{'Related_Open_ticket_info':1,'_id':0}))
        ...
        if  len(relatedOpenTickets) : 
            ...
            for each in relatedOpenIncidentsList:
                exam=list(MongoDBPersistence.rt_tickets_tbl.find({ticket_id_field_name:each},{'_id':0,ticket_id_field_name:1,description_field_name:1}))
                #result_list.append({'description':exam[0]['description'],'number':exam[0]['number']})
                logging.info(f"exam: {exam}")
                result_list.append({'description':exam[0][description_field_name],'number':exam[0][ticket_id_field_name]})
                
            #print("from new Open related tickets ",result_list)    

            if len(result_list) > 0:
                
                #print("open time",stop07-start07)
                return json_util.dumps(result_list)

            else:
                
                #print("open time",stop007-start07)
                result = ["No Results Found"]
                return json_util.dumps(result)
        else :
            logging.info("Realtime Results not found in DataBase ,So we are baking  your results using Word2Vec.py")
            #calling getRelatedTickets in word2vec model
            #we have to get from DB option 
            config_values = MongoDBPersistence.configuration_values_tbl.find_one({}, {"Related_Tickets_Algorithm": 1, "_id": 0}) 
            search_alogorithm=config_values['Related_Tickets_Algorithm']
            print("algorithm selected is ",search_alogorithm)
            if(search_alogorithm=="word2Vec" or search_alogorithm =="doc2Vec" ):
                related_results=RelatedSearch.getRelatedOpenTickets(customer_id, dataset_id, incident_number,search_alogorithm)
    ...
    def getRelatedKnowledgeArticles(customer_id, dataset_id, incident_number, algorithm_name):
        ...
        if (default_kb_source):
            if (default_kb_source["DefaultKBSource"] == "ITSM"):
                ...
                return Recommendation.getSNOWRelatedKnowledgeArticles(customer_id, dataset_id, incident_number, algorithm_name)

            elif (default_kb_source["DefaultKBSource"] == "FileServer"):
                return Recommendation.getFileServerRelatedKnowledgeArticles(customer_id, dataset_id, incident_number, algorithm_name)
    ...
    def getSNOWRelatedKnowledgeArticles(customer_id, dataset_id, incident_number,search_alogorithm, source="SNOW"):
        ...
        try:
            ...
            if (not knowledge_df.empty):
                ...
                # print("Input Description", incident_description)
                ...
                knowledge_df['Summary_cleaned']=knowledge_df.Summary.apply(lambda x: " ".join(re.sub(r'[^a-zA-Z]',' ',w).lower() for w in x.split() if re.sub(r'[^a-zA-Z]',' ',w).lower() not in stopwords) )
                ...
                if(search_alogorithm == 'word2Vec' or search_alogorithm == 'doc2Vec' or search_alogorithm == "BERT"):
                    if(search_alogorithm == 'BERT'):
                        clean_text1 = lambda x: " ".join(re.sub(r'[^a-zA-Z]',' ',w).lower() for w in incident_description.split() if re.sub(r'[^a-zA-Z]',' ',w).lower() not in stopwords)
                        ...
                        if(search_alogorithm == 'word2Vec'):
                            ...
                            sim_scores = KAreleatedsearch.similarity_measures(incident_ticket_descr,descriptions, Word2vecmodel,search_alogorithm, freqs=frequencies)
                            # sim_scores = KAreleatedsearch.word2veccalculate_similarity(incident_ticket_descr, KA_Sum_cleaned)
                        else:
                            doc2Vecmodel = KAreleatedsearch.create_doc2vec_model(KA_Sum_cleaned)
                            sim_scores = KAreleatedsearch.similarity_measures(incident_ticket_descr,descriptions, doc2Vecmodel,search_alogorithm, freqs=frequencies)
                    ...
                    for summary in range(len(knowledge_df['Summary'])):
                        cosine_value.append(Recommendation.sentence_similarity(knowledge_df['Summary'][summary].split(),                                                                            incident_description,
                                                                            stopwords_english))
                    weightDict = {}
                    ...
                    if (weightDict):
                        ...
                        # print("Ranked Knowledge Articles", ranked_knowledge_articles)
                        try:
                        ...
                        # print("Related Articles Number", related_articles)
                        ...
                        # print("Related Articles Summary", related_articles_summary)
                        related_articles_list = []
                        for KB_Number, Summary, Score in zip(related_articles,related_articles_summary,SELECTED_ranked_KA_Score):
                            related_article = {}
                ...
                if (results_matched == 0):
                    ...
                    # print("Related Articles", related_articles_list)
                ...
                if(search_alogorithm == 'word2Vec' or search_alogorithm == 'doc2Vec' or search_alogorithm == "BERT"):
                    if(search_alogorithm == 'BERT'):
                        ...
                        for i in range(2):
                            ...
                            Summary = knowledge_df.iloc[i]['Summary']
                            Score = knowledge_df.iloc[i]['sims']
                            suggestion = {"KBNumber": KBNumber,"Summary":Summary,"Score": float(Score), "Path":KA_path}
                            related_articles_list.append(suggestion)
                    # print(related_articles_list)
                else:
                    for summary in range(len(knowledge_df['Summary'])):
                    ...
                    if (weightDict):
                        ...
                        # list_name = []
                        # list_summary = []
                        # list_name.append(related_articles)
                        # list_summary.append(related_articles_summary)

                        related_articles_list = []

                        # related_articles_dict = dict(zip(list_name, list_summary))
                        #                print('the reated areticle dict is ',related_articles_dict.items())
                        #                print(related_articles_dict)
                        for key, Summary, Score in zip(related_articles, related_articles_summary, SELECTED_ranked_KA_Score):
                            related_article = {}
    ...
    # Fetching knowledge article summary from DB for Word Cloud
    # @staticmethod
    # def getKnowledgeArticlesSummary(customer_id, dataset_id,incident_number):
    #     print('--------------------- in getknowledge api ... -------------------------')
    #     try:
    #         default_kb_source = MongoDBPersistence.configuration_values_tbl.find_one({"CustomerID": customer_id},{"_id": 0, "DefaultKBSource": 1})
    #         if (default_kb_source):
    #             if (default_kb_source["DefaultKBSource"] == "ITSM"):
    #                 related_knowledge_articles = json.loads(Recommendation.getSNOWRelatedKnowledgeArticles(customer_id, dataset_id, incident_number))
    #             elif (default_kb_source["DefaultKBSource"] == "FileServer"):
    #                 related_knowledge_articles = json.loads(Recommendation.getFileServerRelatedKnowledgeArticles(customer_id, dataset_id, incident_number))
    #             elif (default_kb_source["DefaultKBSource"] == "SharePoint"):
    #                 #pass
    #                 # related_knowledge_articles=json.loads(Recommendation.getSharePointRelatedKnowledgeArticles(customer_id, dataset_id, incident_number))
    #                 print('-------kb source is sharepoint---------------')
    #                 related_knowledge_articles=json.loads(Recommendation.getSharePointRelatedKnowledgeArticles())
    #             else:
    #                 logging.info("%s No Valid default KB source is present, please enter KB details again"%RestService.timestamp())
    #         else:
    #             logging.info("%s No default KB source was found, please enter KB details" %RestService.timestamp())
    #             return "Failure"

    #         # related_knowledge_articles_summary = list(MongoDBPersistence.knowledge_info_tbl.find({"CustomerID": customer_id, "DatasetID": dataset_id}, {"Summary":1, "_id":0    }))

    #         if (related_knowledge_articles[0] == "No Results Found"):
    #             logging.info("%s There is no Knowledge Article in DB" %RestService.timestamp())
    #             return "Failure"
    #         else:
    #             related_knowledge_articles_summary = []
    #             # print("In summary", related_knowledge_articles)
    #             for knowledge_article in related_knowledge_articles:
    #                 summary = {}
    #                 summary = {"Summary": knowledge_article['Summary']}
    #                 related_knowledge_articles_summary.append(summary)
    #             # print("In try ", related_knowledge_articles_summary)
    #             return json_util.dumps(related_knowledge_articles_summary)
    #     except Exception as e:
    #         print(str(e))
    #         logging.info("%s Error occured when fetching Knowledge Articles from DB" %RestService.timestamp())
    #         return "Failure"

    # @app.route('/display_resolution/int:customer_id/ticket_number', methods=['GET'])
    @staticmethod
    def display_resolution(customer_id, dataset_id, incident_number, tab_name, cloud_type,algorithm_name):
        # field_mapping = MappingPersistence.get_mapping_details(customer_id)
        # group_field_name = field_mapping['Group_Field_Name']
        ...
        if (field_mapping['Source_ITSM_Name'] == 'SNOW'):
            ...
            close_notes_field_name = 'close_notes'
        # incident_number = 'INC0734175'
        with open('data/stopwords.csv', 'r') as readFile:
            reader = csv.reader(readFile)
            list1 = list(reader)
            # ENGLISH_STOP_WORDS = CSVPersistence.readStopWords(CSVPersistence(config object))
            ...
            try:
                ...
                logging.error("Error occured: Hint: 'assignmentModule' is not defined in 'config/intent.ini' file.")
        ...
        # First we were fetching close notes of KnowledgeArticles
        # rec['close_notes']
        ...
        if (len(resolution[word_cloud_field]) != 0):
            ...
            if (cloud_type == 'Unigram'):
                ...
                # for specified number of words
                # if number_of_words > len(clean_words_dict):
                #         number_of_words = len(clean_words_dict)
    ...
    def getNER(customer_id, dataset_id, incident_number):
        ...
        if (named_entities):
            # required_entities = ['NAME', 'PHONE', 'EMAIL', 'APPLICATION']
            required_entities_lcase = ['Email','Server','Issue','Number','Applications','Platform','Person','Gpe','LeaveApplication','Finance']
            try:
                config = configparser.ConfigParser()
                config["DEFAULT"]['path'] = "config/"
                config.read(config["DEFAULT"]["path"] + "Intent.ini")
    ...
    def getSharePointRelatedKnowledgeArticles(data):
        ...
        try:
            ...
            # fullsp_data=new_list.GetListItems()
            # print(fullsp_data)
            ...
            for i in range(len(searchtext)):
                ...
                # using above list and getting all attached docs for that ids
                # related_article_links=[]
                for i in iD_list:
                    link = data_list.GetAttachmentCollection(i)
                    for i in range(len(link)):
                        related_article_links.append(link[i])
                        # finalresult.append(link[i])
                        # result=related_article_links
            result = related_article_links
            # print(type((related_article_links)))
            # results_matched = len(result)
            # logging.info("%s: Number of search results : %d" % (RestService.timestamp(),results_matched))
            # print('-------result-------------: ',result)
            if len(result) == 0:
    ...
    def update_rt_chosen_algorithm (algorithm_name):
        try:
            ...
            MongoDBPersistence.configuration_values_tbl.update_one({}, {'$set': {'Related_Tickets_Algorithm': algorithm_name}}, upsert=True)
            return 'success'
        except Exception as e:
