...
# model_path = './models/word2vec_pretrained/GoogleNews-vectors-negative300.bin'
...
class KAreleatedsearch:
    ...
    # @staticmethod
    # def KAvectorize(description):
    #     description = description.lower()
    #     words = [w for w in description.split(" ") if w not in stopwords]
    #     word2vec_vecs = []
    #     for word in words:
    #         try:
    #             vec = w2v_model[word]
    #             word2vec_vecs.append(vec)
    #         except KeyError:
    #             pass
    #     vector = np.mean(word2vec_vecs, axis=0)
    #     return vector
    
    # @staticmethod
    # def word2veccalculate_similarity(Tick_desc, KA_Sum=None, threshold=0):
    #     print("word2vec KA search calculate_similarity")
    #     if not KA_Sum:
    #         return []

    #     if isinstance(KA_Sum, str):
    #         KA_Sum = [KA_Sum]

    #     Tick_desc_vec = KAreleatedsearch.KAvectorize(Tick_desc)
    #     results = []
    #     for doc in KA_Sum:
    #         KA_Sum_vec = KAreleatedsearch.KAvectorize(doc)
    #         sim_score = cosine_similarity(Tick_desc_vec.reshape(1, -1), KA_Sum_vec.reshape(1, -1))
    #         if sim_score > threshold:
    #             results.append(sim_score[0][0])
    #     return results
