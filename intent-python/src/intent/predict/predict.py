...
@app.route("/api/predictedfields/<int:customer_id>/<int:dataset_id>")
def getPredictedFields(customer_id, dataset_id):
    return Predict.getPredictedFields(customer_id, dataset_id)
...
# @app.route("/api/getPredictedTicktesForPage/<int:customer_id>/<int:dataset_id>/<int:page_size>/<int:page_num>/<prev_pred_state>")
...
@app.route("/api/assignmentEnableStatus/<field>/<current_status>", methods=["PUT"])
def assignmentEnableStatus(field, current_status):
    return Predict.assignmentEnableStatus(field, current_status)


@app.route("/api/getSwitchStatus", methods=["GET"])
def getSwitchStatus():
    return Predict.getSwitchStatus()


@app.route("/api/hinglishEnableStatus/<field>/<current_status>", methods=["PUT"])
def hinglishEnableStatus(field, current_status):
    return Predict.hinglishEnableStatus(field, current_status)


# @app.route("/api/insertTValue/<tValue>",methods=["POST"])
# def insertTValue(tValue):
#    return Predict.insertTValue(tValue)
@app.route("/api/insertTValue", methods=["POST"])
def insertTValue():
    return Predict.insertTValue()


@app.route("/api/insertIopsStatus/<status>", methods=["POST"])
def insertIopsStatus(status):
    return Predict.insertIopsStatus(status)
...
@app.route("/api/getiOpsValues", methods=["GET"])
def getiOpsValues():
    return Predict.getiOpsValues()
...
@app.route("/api/predictionEnableStatus/<field>/<current_status>", methods=["PUT"])
def predictionEnableStatus(field, current_status):
    return Predict.predictionEnableStatus(field, current_status)
...
class Predict(object):
    ...
    def predictAPI(customer_id, dataset_id, description, pred_field):
        ...
        # confidence_score = float("{0:.2f}".format(max(fittedModel.predict_proba(transformer.transform([description]))[0])))
    ...
    def tick_to_approved(ticket, regex_dictionary):
        ...
        # print("Description Captured for NER is", description_for_ner)
        all_ok_flag = 0
        try:
            dataset_id = ticket['DatasetID']
        ...
        for pred_field in pred_field_list:
            ...
            # print("input_field_list::::::", input_field_list)
            # print("ticket ::::", ticket)
            ...
            if dep_pre['flag'] == 'true' and pred_field in dep_prelist_y:
                ...
                if input_field_list[0] in list_of_dependencies:
                    # print(input_field_list[0],"in the list:",list_of_dependencies)
                    ...
                    # print(input_field_list[0],"not in the list:",list_of_dependencies)
                    ...
                    # print("insde of check list:", input_df)
                ...
                for input_field_ in input_field_list[1:]:
                    ...
                    if input_field_ in list_of_dependencies:
                        # print(input_field_,"in the list:",list_of_dependencies)
                        ...
                        # print(input_field_,"not in the list:",list_of_dependencies)
            ...
            try:
                ...
                for white_lst_doc in white_lst_collection:
                    if (white_lst_doc['PredictedField'] == pred_field and white_lst_doc[
                        'WhiteListed_Words'] != []):
                        # print("ticket description...",input_descr)
                        # print("Whitelisted words...",white_lst_doc['WhiteListed_Words'])
                        for item in white_lst_doc['WhiteListed_Words']:
                            for i in range(len(item['Value'])):
                                if (item['Value'][i]['word']).lower() in input_descr:
                                    # print("word found...",(item['Value'][i]['word']).lower())
    ...
    def predict(customer_id):
        ...
        if (custom_predict_flag == 'failure'):
            ...
            ticket_id_field_name = field_mapping['Ticket_ID_Field_Name']
            status_field_name = field_mapping['Status_Field_Name']
            # assignment_logic_dependancy_field = MongoDBPersistence.assign_enable_tbl.find_one({},{"_id":0,"assignment_logic_dependancy_field":1})
            # print("assignment_logic_dependancy_field",len(assignment_logic_dependancy_field))
            # if (assignment_logic_dependancy_field == '' or len(assignment_logic_dependancy_field) == 0):
            ...
            if (login_user):
                ...
                try:
                    ...
                    logging.error("Error occured: Hint: 'Entities' is not defined in 'config/intent.ini' file.")
                ...
                if spacy_flag:
                    ...
                    nlp = core_web_model
                # move_names = nlp.get_pipe('ner').move_names
                ...
                # MongoDBPersistence.predicted_tickets_tbl.delete_many({'CustomerID':customer_id, "DatasetID": {"$in": accessible_datasets}})
                ...
                try:
                    ...
                    logging.error("Error occured: Hint: 'Multithreading' is not defined in 'config/intent.ini' file.")
                        ...
                        if (len(assignees) == len(pred_assignment_grp_lst)):
                            for index in range(0, len(pred_assignment_grp_lst)):
                                try:
                                    ...
                                    #logging.debug(f"assignee: {assignee}")
                                    final_predictions_lst[index]['predicted_assigned_to'] = assignee
                                    # --Automatic approval of incident tickets based on threshold value--
                                    #logging.debug(f"sub_index: {sub_index}")
                                    #logging.debug(f"total_approve_tickets: {total_approve_tickets}")
                                    if total_approve_tickets > 0:
                                        if (sub_index < total_approve_tickets and t_value_enabled and
                                            ...
                                            #logging.debug(f"tickets_to_be_approved[sub_index][ticket_id_field_name] :{tickets_to_be_approved[sub_index][ticket_id_field_name]}")
                                            #logging.debug(f"final_predictions_lst[index][ticket_id_field_name]: {final_predictions_lst[index][ticket_id_field_name]}")
                ...
                #logging.debug(f"all_ok_flag: {all_ok_flag}")
                all_ok_flag = 1
                if (all_ok_flag):
                    try:
    ...
    # @app.route("/updateuserstatus/<int:customer_id>/<int:dataset_id>/<threshold_value>",methods=["GET"])
    @staticmethod
    ...
    # @app.route("/predictedTickets/<int:customer_id>/<int:dataset_id>",methods=["GET"])
    @staticmethod
    def getPredictedTickets(customer_id, dataset_id):
    ...
    # @app.route("/predicted/<int:customer_id>/<int:dataset_id>/<ticket_id>",methods=["GET"])
    @staticmethod
    def getPredictedData(customer_id, dataset_id, ticket_id):
    ...
    # @app.route("/predictedfields/<int:customer_id>/<int:dataset_id>")
    @staticmethod
    def getPredictedFields(customer_id, dataset_id):
        ...
        try:
            ...
            if (session.get('user')):
                ...
                if (role == 'Admin'):
                    ...
                    user_doc = MongoDBPersistence.users_tbl.find_one({'UserID': user})
                    #logging.debug(f"user_doc:{user_doc}")
                    if (user_doc):
                        team_id = int(user_doc['TeamID'])
                        team_doc = MongoDBPersistence.teams_tbl.find_one({'TeamID': team_id})
                        #logging.debug(f"team_doc: {team_doc}")
                        if (team_doc):
                            dataset_id = team_doc['DatasetID']
            ...
            #logging.debug(f"datasetID: {datasetID}")
        except Exception as e:
            logging.error('%s: Error occurred: %s ' % (RestService.timestamp(), str(e)))
            logging.error(e, exc_info=True)
            datasetID = "Failure"
    ...
    def getPredictedTicketsForPage(customer_id, dataset_id, page_size, page_num, prev_pred_state, show_app_ticket):
        ...
        try:
            ...
            if (session.get('user')):
                ...
                return "No Login Found"
            # ----------------------------------------
            # match_doc = {'CustomerID': customer_id, "DatasetID":dataset_id,"user_status":"Not Approved"}
            ...
            #            result = list(MongoDBPersistence.predicted_tickets_tbl.find({'CustomerID': customer_id, "DatasetID":dataset_id}).skip(skips).limit(page_size))
            ...
            try:
                ...
                if (confidenceScore is not None):
                    ...
                    for incident in result:
                        #logging.info(f"incident: {incident}")
            ...
            if (prediction_to_show):
                ...
                #                print("Incidents to map", incidents_to_map)
                final_result = []
                for index, incident in enumerate(prediction_to_show):
                    #logging.info(f"index: {index}")
                    #logging.info(f"incident: {incident[ticket_id_field_name]}")
                    for rt_incident in incidents_to_map:
                        #logging.info(f"rt_incident: {rt_incident[ticket_id_field_name]}")
                        #logging.info(f"incident: {incident[ticket_id_field_name]}")
                        if ticket_id_field_name not in rt_incident:
                            rt_incident[ticket_id_field_name] = rt_incident[ticket_id_field_name]
                        # if 'description' not in rt_incident:
                        #    rt_incident['description'] = rt_incident[description_field_name]
    ...
    def assignmentEnableStatus(field, current_status):
        ...
        #            config["DEFAULT"]['path'] = "config/"
        #            config.read(config["DEFAULT"]["path"]+"Intent.ini")
        #            logging.info('%s: trying to update value of "assignmentModule" of ini file'%RestService.timestamp())
        ##            config["ModuleConfig"]["assignmentModule"]
        #            if(current_status=='true'):
        #                config["ModuleConfig"]["assignmentModule"]='yes'
        #            else:
        #                config["ModuleConfig"]["assignmentModule"]='no'
        except Exception as e:
            logging.error('%s: Error: %s' % (RestService.timestamp, str(e)))
            return "Failure"
    ...
    def insertTValue():
        try:
            ...
            #            if(tValue!='null' and tValue!=0):
            if (t_value_doc != 'null'):
                MongoDBPersistence.assign_enable_tbl.update_one({}, {'$set': {'threshold_value': t_value_doc}},
                                                                upsert=True)
#            else:
#                MongoDBPersistence.assign_enable_tbl.update_one({},{'$unset':{'threshold_value':1}})
            ...
            for cust_id in cust_ids:
                for datast_id in dataset_ids:
                    try:
                        ...
                        logging.info("User status updated for Customer_ID %s and DatasetID %s" % (str(cust_id),str(datast_id)))
                    except:
                        logging.info("%s Customer_ID or DatasetID is not presented in predictdata table" % RestService.timestamp())
        except Exception as e:
            logging.error('%s: Error: %s' % (RestService.timestamp, str(e)))
            return "Failure"
        return "Success"
    ...
    def insertIopsStatus(status):
        try:
            ()
            if (status != None):
                MongoDBPersistence.assign_enable_tbl.update_one({}, {'$set': {'iOpsStatus': status}}, upsert=True)
        #            else:
        #                MongoDBPersistence.assign_enable_tbl.update_one({},{'$unset':{'threshold_value':1}})
        except Exception as e:
            logging.error('%s: Error: %s' % (RestService.timestamp, str(e)))
    ...
    def getiOpsValues():
        try:
            ()
            source = MongoDBPersistence.assign_enable_tbl.find_one({}, {"iOpsStatus": 1, 'iOpsPath': 1, "_id": 0})
    ...
    def importantFeaturesReport(customer_id, dataset_id, number):
        ...
        try:
            ...
            ##algo name by customer and dataset_id
            field_mapping = MappingPersistence.get_mapping_details(customer_id)
            ticket_id_field_name = field_mapping['Ticket_ID_Field_Name']
            cust_name = \
            MongoDBPersistence.customer_tbl.find_one({"CustomerID": customer_id}, {"CustomerName": 1, "_id": 0})[
            ...
            if (number == 'INC000105843836'):
                weight_final['assignment_group'] = [['po'], ['vendor']]
                weight_final['category'] = [['create'], ['material']]
            elif (number == 'INC000105491075'):
                weight_final['assignment_group'] = [['shipment '], ['number']]
                weight_final['category'] = [['track'], ["trace"], ['tool']]

            elif (number == 'INC000105625650'):
                weight_final['assignment_group'] = [['report'], ['account'], ['invoice']]
                weight_final['category'] = [['sap'], ['invoice'], ['electronic']]

            elif (number == 'INC0010305'):
                weight_final['assignment_group'] = [['AWSManagementPortal']]
                weight_final['category'] = [['application'], ['raised'], ['region'], ['request']]

            elif (number == 'INC02909781'):
                weight_final['assignment_group'] = [['delivery'], ['PO']]
                weight_final['category'] = [['perform'], ['PO'], ['check']]

            elif (number == 'RQ61470034'):
                weight_final['assignment_group'] = [['variable'], ['compensation'], ['appropriate'], ['update']]
                weight_final['category'] = [['variable'], ['compensation'], ['appropriate'], ['calculated'], ['pay'],
                                            ['salary']]

            elif (number == 'SR657898351'):
                weight_final['assignment_group'] = [['delivery'], ['Pos']]
                weight_final['category'] = [['delivery'], ['perform'], ['Pos'], ['completion']]

            elif (number == 'RQ61559134'):
                weight_final['assignment_group'] = [['variable'], ['payouts'], ['percentage'], ['quarter'],
                                                    ['Consulting'], ['DU']]
                weight_final['category'] = [['variable'], ['payouts'], ['percentage'], ['rating'], ['least'],
                                            ['billing'], ['eligible']]

            elif (number == 'INC0010230'):
                weight_final['assignment_group'] = [['AWSManagementPortal'], ['space'], ['disk']]
                weight_final['category'] = [['application'], ['fscing'], ['remote'], ['space'], ['raised'],
                                            ['including'], ['commercial'], ['issues'], ['assist'], ['replacing']]
    ...
    def sortAndFilter(cstmr_id, dataset_id, fltr_fld, fltr_val, srt_fld, srt_ordr, page_size, page_num,
        ...
        try:
            ...
            if (str(fltr_fld) != 'undefined'):
                ...
                #                    doc_count = MongoDBPersistence.rt_tickets_tbl.find({fltr_fld:fltr_val}).count()
                else:
                    match_doc[fltr_fld] = {'$regex': fltr_val, '$options': 'i'}
            #                    doc_count = MongoDBPersistence.predicted_tickets_tbl.find({fltr_fld:fltr_val}).count()
            if (prev_pred_state == 'false'):
                match_doc['user'] = {'$exists': False}
            ...
            for pred_doc in pred_doc_lst:
                ...
                # if 'description' not in pred_doc:
                #    pred_doc['descripion'] = pred_doc[description_field_name]
    ...
    def itsm_users(customer_id, method):
        ...
        try:
            ...
            #            elif(method == 'post'):
            #                mapped_lst = request.get_json()
            #                users  = MongoDBPersistence.itsm_user_mapping.update(mapped_lst)
    ...
    def tickets_assigned_to_user(customer_id, dataset_id, page_size, page_num, show_app_ticket):
        try:
            ...
            if (user == "admin"):
                ...
                # return json_util.dumps(data_to_send)
            else:
