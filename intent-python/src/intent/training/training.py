...
from sklearn.model_selection import  GridSearchCV #,train_test_split # GridSearchCV has to be used in future
#from sklearn.base import BaseEstimator, TransformerMixin
from sklearn.ensemble import RandomForestClassifier
from sklearn.linear_model import LogisticRegression  
#from sklearn.cluster import KMeans     #Can be used in future
...
class Training(object):
    ...
    #@app.route("/algoConfiguration/<int:customer_id>/<int:dataset_id>/<status>", methods=['GET'])
    @staticmethod
    def algoConfiguration(customer_id, dataset_id, status):  #Status will be eigher Tuning/Default
        ...
        if(dataset_name):
            ...
            if(status=="Tuning"):
                ...
                #last_tuning_dict = training_hist_tbl.find_one({"CustomerID" : customer_id,"DatasetID" : dataset_id,"TrainingStatus" : "Tuning", "TrainingID": {"$exists": True}},{'_id':0,"TrainingID":1}, sort=[("TrainingID", -1)])
                #last_inprogress_dict = training_hist_tbl.find_one({"CustomerID" : customer_id,"DatasetID" : dataset_id,"TrainingStatus" : "InProgress", "TrainingID": {"$exists": True}},{'_id':0,"TrainingID":1}, sort=[("TrainingID", -1)])
                #if(last_tuning_dict and last_inprogress_dict):
                #    last_tuning_id = last_tuning_dict['TrainingID']
                #    last_inprogress_id = last_inprogress_dict['TrainingID']
                #else:
                    #It should be non-Registered Customer
                #    logging.info('%s: Failure: Last Tuning training info not found for the customer.'%RestService.timestamp)
...
#==========================================================================================================
                ...
                if stopwords_list:
                    ...
                    try:
                        ...
                        # print("changing Inprogress to approved")
                        #logging.info('%s: All the InProgress stopwords are now approved, Table approved successfully'%RestService.timestamp())
...
#==========================================================================================================
                ...
                # else:
                #     logging.info('%s: Choices selected previously are saved successfully. No need to resubmit the data again.'%RestService.timestamp())
                #     resp = 'success'
            elif(status=="Default"):
                #Update Training status
                ...
                #Recieving last training ID to update TblTraining
                #last_training_dict = training_hist_tbl.find_one({"CustomerID" : customer_id,"DatasetID" : dataset_id,"TrainingStatus" : "Tuning", "TrainingID": {"$exists": True}},{'_id':0,"TrainingID":1}, sort=[("TrainingID", -1)])
                #if(last_training_dict):
                #    last_training_id = last_training_dict['TrainingID']
                #    try:
                #        training_hist_tbl.update_one({'CustomerID':customer_id,"TrainingID":last_training_id}, {"$set": {"TrainingStatus":"Archived"}}, upsert=False)
                #        logging.info('%s: Updated: TblTraining'%RestService.timestamp())
                #    except Exception as e:
                #        logging.error('%s: Error occurred: %s '%(RestService.timestamp(),str(e)))
                #else:
                #    logging.info('%s: Info: it seems customer didnt tuned the algos.'%RestService.timestamp())
                #Update both the statuses in TblCustomer & TblTraining
...
# ================================================================================================================================
    ...
    #@app.route("/train/<int:customer_id>/<int:dataset_id>", methods=['POST'])
    @staticmethod
    def train(customer_id, dataset_id):
        ...
        if (custom_train_flag=='failure'):
            ...
            # logging.info("%s Custom train is present invoking Custom ITSM"%RestService.timestamp())
            # api = custom_url + "api/custom_train/"+str(customer_id)+"/"+str(dataset_id) 
            # proxies = {"http": None,"https": None}
            # req_response = requests.get(api,proxies=proxies)
            # print("req response ",req_response , "and value ",req_response.json())
            ...
            #post_data = {"CustomerID": self.customer_id,"DatasetID":self.dataset_id,"data":user_param_choices['Status'], "user_id": user}
            ...
            # print("__ custom response text  ",req_res.text)
            # print("req response ",req_response , "and value ",req_response.json())
            ...
            # training_data =  list(training_tickets_tbl.find({'CustomerID':customer_id}))
            # if(not training_data):
            #    logging.info('%s: Failure: Training data is not available in TblIncidentTraining. Upload the training data & try again.'%timestamp)
            #    return "failure"
            ...
            # last_training_id = training_hist_tbl.count_documents({"CustomerID" : customer_id})
            # print(last_training_id)
            ...
            # cust_name =  list(customer_tbl.find({'CustomerID':customer_id},{'CustomerName':1,'_id':0}))[0]['CustomerName']
            ...
            for pred_field in pred_field_list:
                ...
                #training_tkt_df = training_tkt_df[pd.notnull(training_tkt_df[pred_field])]
                ...
                if (custom_processing_flag=='failure'):
                    ...
                    try:
                        ...
                        #raise ValueError('No Custom preprocessing file present for this account'+e)
                else:
                    training_tkt_df = IncidentTraining.cleaningInputFields(training_tkt_df,in_field)
                    ...
                    for algo in user_param_choices['Algorithms']:
                        ...
                        if (user_param_choices['Status']=='Tune' and algo_name == 'XGBClassifier'):
                            if (algo['ParameterType'] == "Multiple"):
                                ...
                                # print(score_dict['F1_score'])
                                # print(score_dict['Precision'])
                                # print(score_dict['Recall'])
                                logging.info('%s: Model has been trained successfully using %s algorithm.' % (RestService.timestamp(), algo_name))
                            ...
                            if (algo['ParameterType'] == "Multiple"):
                                ...
                                #print("Param grid: %s"%param_grid)   #hp.quniform('max_depth', param_grid['max_depth'], param_grid['max_depth'],1)
            ...
            # training_flag = [1 for i in range(len(training_tkt_df.index))]
            ...
            try:
                ...
                #--Code to insert records to TblWhitelistedWordDetails according to predicted fields--(sijo)---------------
                if(training_status != "Tuning"):
                    whitelisted_record={}
                ...
                return json_util.dumps('failure')
            # response = "Model trained successfully using "+algoName+" algorithm "+"with accuracy="+str(accuracy)
    ...
    #@app.route('/displayGraph/<int:customer_id>/<int:dataset_id>/<predicted_field>', methods=["GET"])
    @staticmethod
    def displayGraph(customer_id,dataset_id,predicted_field):
        #print("Control is here 1")
    ...
    #Code for word cloud - Adding new stop words
    #@app.route('/saveStopWords/<int:customer_id>/<int:dataset_id>',methods = ['PUT'])
    @staticmethod
    def saveStopWords(customer_id, dataset_id):
        ...
        if stopwords_list:
            #stopwords_list = {'Stopwords': ['abc','xyz']}
            #stopwords_list = stopwords_list['Stopwords']
            ...
            # db.TblDataset.update_one({"$and":[{"DatasetID": dataset_id},{"CustomerID": customer_id}]},
            # {"$set": {"Stopwords": stopwords_list}}, upsert=False)
            #logging.info('%s: New Stopwords addedd successfully...'%RestService.timestamp())
            ...
            # db.TblDataset.update_one({"$and":[{"DatasetID": dataset_id},{"CustomerID": customer_id}]},
            # {"$set": {"Stopwords": stopwords}}, upsert=False)
            #logging.info('%s: No previous stopwords found, updating stopwords with new stopwords'%RestService.timestamp())
        return json_util.dumps(stopwords)  #not sure about what to return

    # code for getting json for Wordcloud
    #returns a unigram json
    #@app.route('/displayWordCloud/<int:customer_id>/<int:dataset_id>/<predictedfield>/<path:sub_field>/<int:number_of_words>/<cloud_type>', methods = ['GET'])
    @staticmethod
    def displayWordCloud(customer_id, dataset_id, predictedfield,sub_field,number_of_words,cloud_type):
    ...
    #returns a list of stopwords to be displayed
    #@app.route('/displayStopWords/<int:customer_id>/<int:dataset_id>',methods=['GET'])
    @staticmethod
    def displayStopWords(customer_id,dataset_id):
        stopwords_list = MongoDBPersistence.datasets_tbl.find_one({"CustomerID": customer_id,"DatasetID": dataset_id},{"Stopwords":1,"_id":0})
        ()
    ...
    # remove a clicked stopword
    #@app.route('/removeStopWords/<int:customer_id>/<int:dataset_id>/<word>', methods=['GET'])
    @staticmethod
    def removeStopWords(customer_id,dataset_id,word):
        stopwords_list = MongoDBPersistence.datasets_tbl.find_one({"CustomerID": customer_id,"DatasetID": dataset_id},{"Stopwords":1,"_id":0})
        ()
        if stopwords_list:
            ...
            if word in approved_stopwords:
                ...
                print('error')
                #logging.info('%s:Stopword not found...'%RestService.timestamp())
            MongoDBPersistence.datasets_tbl.update_one({"$and":[{"DatasetID": dataset_id},{"CustomerID": customer_id}]},
            {"$set": {"Stopwords": [{"stopword": in_progress_stopwords,"Status": 'InProgress'}, 
            {"stopword": approved_stopwords, "Status": 'Approved'}]}}, upsert=False)
            # db.TblDataset.update_one({"$and":[{"DatasetID": dataset_id},{"CustomerID": customer_id}]},
            # {"$set": {"Stopwords": stopwords_list}}, upsert=False)
            #logging.info('%s:Stopword removed successfully...'%RestService.timestamp())
        else:
            #logging.info('%s: There were no stopwords to be removed...'%RestService.timestamp())
            print("hello")
    ...
    #************ moveToTrainingData: Work in Progress *******************
    #@app.route("/moveToTrainingData/<int:customer_id>/<int:dataset_id>",methods=["POST"])
    @staticmethod
    def moveToTrainingData(customer_id, dataset_id):
    ...
    #Dont call this, instead call above method
    #@app.route("/updateTrainingStatus/<int:customer_id>/<status>",methods=["PUT"])   #Try Once soma is able to pass TrainingID from Angular
    @staticmethod
    def update_status(customer_id,status):
    ...
    @staticmethod
    def getClassificationReport(customer_id, dataset_id, predicted_field, algorithm, training_status):
        #print(customer_id, dataset_id, predicted_field, algorithm, training_status)
        output_lst = []
    ...
    def updateWhiteListWords(customer_id, dataset_id, predicted_field):
        ...
        #print("White Words List", white_words_lst)
        # white_words_data= {'words': [{'field_name':'sap fisco','keyword':'test2'},{'field_name':'sap bi','keyword':'bi test1'}, {'field_name':'SAP Basis Support','keyword':'test1'}]}
        white_words_data = request.get_json()
        # white_words_data= {'words': [{'field_name':'sap fisco','keyword':'test2', 'weightage': 1.0},{'field_name':'sap bi','keyword':'bi test1'}, {'field_name':'SAP Basis Support','keyword':'test1'}]}

        print(white_words_data)
        print(type(white_words_data))
        white_words_dict = defaultdict(list)
        resp = 'success'
            #white_words_temp = request.get_json()
        ...
        for field, words in white_words_dict.items():
            if (white_words_lst):
                    ...
                    #     if(white_word not in white_words_lst):
                    #         validated_white_words.append(white_word)
                ...
                # old_white_words = {'WhiteListed_Words': [{'Field_Name': 'SAP SD Support',
                #                     'Value': ['server_stop', 'loginissue', 'job_failed']},
                #                     {'Field_Name': 'SAP Basis Support',
                #                     'Value': ['SAP_ERP', 'distributionPrd', 'sd_sales']}]}
                if (old_white_words):
                    try:
    ...
    def getWordCloudList(customer_id,dataset_id,predicted_field,number_of_words,cloud_type):
        ...
        #print("subcategory " ,sub_category_labels)
        word_cloud_list =[]
        word_count_dict={}
...
#        tmp_dict={}
#        ##creating a tuple
#        col_list = (predicted_field,"Words","Count")
#        tmp_dict ={}
#        tmp_dict[col_list]  = "columns"
#        data_list.append(tmp_dict.keys())
#        print("data_list " ,data_list)
        si = StringIO()
        cw = csv.writer(si)
        cw.writerows(data_list)
        #print("si values " ,si.getvalue())
    ...
    def getBarCategoryList(customer_id,dataset_id):
        ...
        # print(category_list[0]['Bar_Filter_Columns'])
    ...
    def getPieCategoryList(customer_id,dataset_id):
        ...
        # print(category_list[0]['PieChart_Filter_Columns'])
        print("printing dropdown values from DB ",dropdownvalues)
        return json_util.dumps(dropdownvalues)
    ...
    def getSourceTargetColumns():
        ...
        try:
            # log_setup()
            # source_columns=list(MongoDBPersistence.datasets_tbl.find({},{'_id':0, "ColumnNames": 1}))
            ...
            # source_list = source_columns[0]['ColumnNames']
