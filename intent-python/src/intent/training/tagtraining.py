...
class TagsTraining(object):
    ...
    #@app.route('/uploadTrainingData/<int:customer_id>/<dataset_name>/<team_name>', methods=["POST"])
    @staticmethod
    def db_insert_tags_training_tickets( customer_id,dataset_name,team_name):
    ...
    #        description_field_name = field_mapping['Description_Field_Name']
            #print('Inside Insert')
            #trainDB = db["TblIncidentTraining"]
            #csv file of users
            #f = self.request.body
            new_dataset_flag = 0
            file = request.files['trainingData']
            dataset_ = MongoDBPersistence.teams_tbl.find_one({"CustomerID": customer_id,"TeamName":team_name},{"TagsDatasetID":1,"_id":0})
            if(dataset_):
                dataset_dict = {}
                ...
                #o_dataset_name = datasets_tbl.find_one({"CustomerID": customer_id,"DatasetID":dataset_id},{"DatasetName":1,"_id":0})
            else:
                ...
                dataset_dict = MongoDBPersistence.tag_dataset_tbl.find_one({"CustomerID" : customer_id, "TagsDatasetID": {"$exists": True}},{'_id':0,"TagsDatasetID":1}, sort=[("TagsDatasetID", -1)])
                if(dataset_dict):
        ...
        #latin-1
            ...
            #for item in csv_dicts:
                #tem.update( {"CustomerID":customer_id})
            ...
            # trainign_row = MongoDBPersistence.training_tickets_tbl.find_one({'CustomerID':customer_id,'DatasetID':dataset_id},{'_id':0})
            # if(trainign_row):
                # prev_uploaded_cols = list(trainign_row)
                # if(not set(csv_df_cols) == set(prev_uploaded_cols)):
                #     logging.info('%s: Column names of previously uploaded data dont match with uploaded csv.'%RestService.timestamp())
                #     return 'failure'
            
            # for col in tbl_columns:
            #     if(input_tickets_df.col.dtype=='object' or input_tickets_df.col.dtype=='string'):
            #         for index,row in input_tickets_df.iterrows():
            #             row[col] = get_clean_text(row[col])
            #             row[col] = remove_special_char(row[col])
            current_row_count = df.shape[0]
            prev_ticket_count = MongoDBPersistence.tag_dataset_tbl.find_one({"CustomerID": customer_id,"TagsDatasetID":dataset_id},{"TicketCount":1,"_id":0})
    ...
    @staticmethod
    # def get_data(tag_column,tag_based_column):
    def get_data():
        print("In get data function. Below are the column names")
        # print(tag_column)
        # print(tag_based_column)
        try:
            ()
            ...
            #return df[[tag_column, tag_based_column]]
            return df
        except Exception as e:
            logging.error('%s: Error occurred: %s '%(RestService.timestamp(),str(e)))
    ...
    '''
    def get_vectorized_representation(text,customer_name,dataset_name):
        path='models/tagging/'+customer_name+'__'+dataset_name+'__'+"tfidf__tags__vector.pkl"
        vectorizer = TfidfVectorizer(strip_accents='unicode', analyzer='word', ngram_range=(1,3), 
                                  norm='l2', vocabulary=pickle.load(open(path, "rb")))
        vectorized_text = vectorizer.fit_transform(text)
        terms = vectorizer.get_feature_names()
    ...
    def load_and_predict(vectorized_text):
        ...
        '''
    def load_and_predict(vectorized_text,customer_name,dataset_name):
        model_loaded = joblib.load('models/tagging/'+customer_name+'__'+dataset_name+'__model__tags__tagging.sav')
        predictions = model_loaded.predict(vectorized_text)
    ...
    def get_association(predicted_tags):
        ...
        filtered_rule_df = rule_df.loc[rule_df['element1'].isin(predicted_tags) | rule_df['element2'].isin(predicted_tags)]
        ...
        for i, row in sorted_rule_df.iterrows():
            if 'Root Cause' in row['element1'] and row['element1'] not in unique_tags:
    ...
    def CustomTags(desc,customer_id,dataset_id):
        try:
            ...
            print(desc_list)
            cluster_list=[]
#            MongoDBPersistence.cluster_details_tbl.find_one({"cluster_details.values": "value2" },{ "array.$": 1, _id : 0 })
            details=MongoDBPersistence.cluster_details_tbl.find_one({'customer_id':customer_id,'dataset_id':dataset_id,},{'_id':0, 'cluster_details':1})['cluster_details']
            for clus in details:
                print(type(clus))
                if('customValues' in clus):
                    #print("type of db clus...",type(clus['customValues']))
                    #print("type of desc...",type(desc_list))
    ...
    def get_association(predicted_tags):
        with open("models/tagging/association_rules_with_resolution.pkl", "rb") as f:
            rule_df = pickle.load(f)
        #print("Rule dataframe")
        #print(rule_df.shape)
        filtered_rule_df = rule_df.loc[rule_df['element1'].isin(predicted_tags) | rule_df['element2'].isin(predicted_tags)]
        #print("filtered rule dataframe")
        #print(filtered_rule_df.shape)
        sorted_rule_df = filtered_rule_df.sort_values(by=['Confidence'], ascending=False)
        #print("Sorted rule dataframe")
        #print(sorted_rule_df.shape)
        unique_tags = []
        adjacant_confidence_scores = []
        for i, row in sorted_rule_df.iterrows():
        ...
        #print("adjacant_confidence_scores:", adjacant_confidence_scores)
        final_list = {}
        for tag, score in zip(unique_tags, adjacant_confidence_scores):
            final_list[tag] = score
            print(tag, score)
    ...
    @staticmethod
    def Tags(incident_no,customer_id,dataset_id):
        # short_description = "CTMPRD sb_trd_fundbalcheck SB-Siebel SBL81PRD hsiebp01 1 3ejn7 running out of disk space"
        # category = "Internal Applications"
        # sub_category = "Tradewinds"
        # short_description = request.form.get('ticket_short_description')
        # category = request.form.get('ticket_category')
        # sub_category = request.form.get('ticket_subcategory')
        try:
            # print(incident_no,',',customer_id,',',dataset_id)
            rt_doc = MongoDBPersistence.rt_tickets_tbl.find_one({'CustomerID':customer_id,'DatasetID':dataset_id,'number':incident_no},{'_id':0,'description':1,'subcategory':1,'category':1})
            all_clms = MongoDBPersistence.datasets_tbl.find_one({'CustomerID':customer_id,'DatasetID':dataset_id},{'_id':0,'ColumnNames':1})['ColumnNames']
            #print(all_clms)
            #print(type(all_clms))
            # print('------rt doc =---------',rt_doc)
            ...
            #if('close_notes' in rt_doc.keys()):
            #    close_note = rt_doc['close_notes']
    
            # print("Received ticket short description:: ", short_description)
            # print("Received ticket category:: ", category)
            # print("Received ticket sub-category:: ", sub_category)
            ...
            one_hot_columns, features = TagsTraining.get_combined_text_and_categorical_features(vectorized_text, [category, sub_category])
            ...
            # clstr_no = predicted_cluster[0]
            # print('--clstr no-- : ',clstr_no)
            
            # cluster_list=TagsTraining.CustomTags(short_description,customer_id,dataset_id)
            # predicted='Cluster '+str(clstr_no)
            # cluster_list.append(predicted)
            # cluster_list=list(set(cluster_list))
            
            # return json_util.dumps(cluster_list)


            # get features from the text example which contributed into predicting the cluster(top 5 only for now)
            predicted_tags = TagsTraining.get_predicted_features(terms+one_hot_columns, model_loaded.cluster_centers_, features)
            predicted_tags = list(dict(predicted_tags).keys())[:5]
            print("Predicted tags from the predicted cluster:", predicted_tags)
            #str(list(dict(predicted_tags).keys())[:5])

            # print(predicted_tags)
            #print("Keys:", rt_doc.keys())
            split_count = 0
...
#        clstr_details_doc = MongoDBPersistence.cluster_details_tbl.find_one({'customer_id':customer_id, 'dataset_id':dataset_id},{'_id':0, 'cluster_details':1})
#        if(clstr_details_doc):
#            predicted_tags = clstr_details_doc['cluster_details'][clstr_no]['values']
#            predicted_tags = predicted_tags[:5]
#            print(predicted_tags)
#
#            predicted_tags_cleaned = []
#            for i in predicted_tags:
#                if len(i.split("_")) > 1:
#                    predicted_tags_cleaned.append("".join(i.split("_")[1:]))
#                else:
#                    predicted_tags_cleaned.append(i)
#
#
#            return json_util.dumps({'Tags': predicted_tags_cleaned})
#        else:
#            return "tag_generation_failed"
    '''
    def Tags(customer_id,dataset_id):
    # def Tags(desc,customer_id,dataset_id):
        try:
            short_description = "CTMPRD sb_trd_fundbalcheck SB-Siebel SBL81PRD hsiebp01 1 3ejn7 running out of disk space"
            category = "Internal Applications"
            sub_category = "Tradewinds"
...
#            sub_category = request.form.get('ticket_subcategory')
            ...
            #get vectorized representation of the data
            terms, vectorized_text = TagsTraining.get_vectorized_representation([text],customer_name,dataset_name)
            
            one_hot_columns, features = TagsTraining.get_combined_text_and_categorical_features(vectorized_text, [category, sub_category])
            
            #load the model and make predictions
            model_loaded, predicted_cluster  = TagsTraining.load_and_predict(features,customer_name,dataset_name)
            
            predicted_tags = TagsTraining.get_predicted_features(terms+one_hot_columns, model_loaded.cluster_centers_, features)
            predicted_tags = list(dict(predicted_tags).keys())[:5]
            
            return 'Ticket description(cleaned): ' + text + "<br>Category: " + category + "<br>Sub-category: " + sub_category + "<br>Tags: <br>" + str(predicted_tags)
            #load all the tag names
#            loaded_cols = pickle.load(open('models/tagging/'+customer_name+'__'+dataset_name+'__'+'total__tags__file.pkl', "rb"))
#            
#            index = 1
#            predicted_tags = []
#            for i in predictions.getnnz(0):
#                if i == 1:
#                    tag = str(loaded_cols[index]).split("_")[1]
#                    predicted_tags.append(tag)
#                    print("Index: {}, Predicted tag: {}".format(index, tag))
#                
#                index += 1
#                
#            return json_util.dumps(predicted_tags)
        except Exception as e:
        ...
        for col in one_hot_columns:
            for val in test_categorical_values:
                if val == col[col.find("_")+1:]:
                    #print("index: {0}, value:{1}, match:{2}, column:{3}".format(index, val, col[col.find("_")+1:], col))            
    ...
    def get_combined_text_and_categorical_features(vectorized_text, test_categorical_values):
        #load all the one-hot-encoded columns
        one_hot_columns = pickle.load(open("models/tagging/AMD__Infra Team__tfidf__tags__vector.pkl", "rb"))
    ...
    def getTagBasedField(customer_id,dataset_id):
        ...
        #result = customer_tbl.find_one({"CustomerID": customer_id},{"PredictedFields":1,"_id":0})
        try:
            ()
            result = MongoDBPersistence.tag_dataset_tbl.find_one({"CustomerID": customer_id, "TagsDatasetID":dataset_id},{"TagBasedField":1,"_id":0})
            if(result):
                resp = json_util.dumps(result)
                logging.info('%s: Getting TagBasedField of the customer...'%RestService.timestamp())
            else:
                resp = ''
                logging.info('%s: TagBasedField not found in TblTagDataset, returning empty string...'%RestService.timestamp())
            return resp
        except Exception as e:
            logging.error('%s: Error: %s'%(RestService.timestamp,str(e)))
            return 'Failure'
    ...
    @staticmethod
    def process_categorical_variables(data, tfidf_vectors, categorical_variables):
        features = pd.DataFrame(tfidf_vectors.todense())
        print("dense tfidf shape:", features.shape)
        print("dense tfidf index", features.index)
        del tfidf_vectors
        dummies = pd.get_dummies(data=data, columns=categorical_variables)
    ...
    def tagTrain(customer_id, dataset_id):
        ...
        try:
            ...
            categorical_variables = ['category', 'subcategory']
            
            #drop null values
            df = df.dropna(how='any',axis=0) 
            
            df['description'] = df['description'].str.lower()
            df['description'] = df['description'].apply(TagsTraining.cleanHtml)
            df['description'] = df['description'].apply(TagsTraining.cleanPunc)
            df['description'] = df['description'].apply(TagsTraining.keepAlpha)
            df['description'] = df['description'].apply(TagsTraining.remove_stopwords)
            df['description'] = df['description'].apply(TagsTraining.lemmatize_text)


            #correct the index of the dataframe in case of any disturbance
            df.index = range(0, df.shape[0])
            
            vectorizer = TfidfVectorizer(stop_words='english')
            X = vectorizer.fit_transform(df['description'])
            
            with open("models/tagging/tfidf_vector_clustering_ShortDescription_3Fields.pkl","wb") as f:
                pickle.dump(vectorizer.vocabulary_, f)
            print("The vocabulary has been saved with the name: \
                  tfidf_vector_clustering_ShortDescription_3Fields.pkl in the current directory.")

            dummies_2 = pd.get_dummies(data=df[categorical_variables], columns=categorical_variables)

            #save one-hot columns for the future predictions on single example
            with open("models/tagging/one_hot_columns_3Fields.pkl","wb") as f:
                pickle.dump(list(dummies_2.columns), f)
            
            #combine categorical and text features into one unit
            featuresss = TagsTraining.process_categorical_variables(df[categorical_variables], X, categorical_variables)
            
            #train
            modelkmeans = KMeans(n_clusters=14, verbose=True)
            modelkmeans.fit(featuresss)
            
            import joblib
            joblib.dump(modelkmeans, 'models/tagging/model_clustering_3Fields.sav')
            print("Model is saved in the current folder. File name is model_clustering_3Fields.sav")



            if 'close_notes' in df.columns:
                # print("Close notes column is there.===============")
                categorical_variables = ['category', 'subcategory', 'close_notes']
                dummies_2 = pd.get_dummies(data=df[categorical_variables], columns=categorical_variables)
                with open("models/tagging/one_hot_columns_4Fields.pkl","wb") as f:
                    pickle.dump(list(dummies_2.columns), f)
                featuresss = TagsTraining.process_categorical_variables(df[categorical_variables], X, categorical_variables)
                modelkmeans = KMeans(n_clusters=14, verbose=True)
                # print("started running training association model now.")
                modelkmeans.fit(featuresss)
                # print("Association training completed. Rules will be prepared now.")
                
                number_of_features_to_be_taken = 20
                order_centroids = modelkmeans.cluster_centers_.argsort()[:, ::-1]
                terms = vectorizer.get_feature_names()
                with open("models/tagging/one_hot_columns_4Fields.pkl", "rb") as f:
                    one_hot_columns = pickle.load(f)
                total_terms = terms + one_hot_columns
                
                predicted_features = []
                for i in range(14):
                    #print("Cluster %d:" % i),
                    current_cluster_features = []
                    for ind in order_centroids[i, :number_of_features_to_be_taken]:
                        #print(' %s' % total_terms[ind]),
                        current_cluster_features.append(total_terms[ind])
                    predicted_features.append(current_cluster_features)
                    
                import itertools

                support = 0.0045
                confidence = 10
                D, L1 = TagsTraining.compute_parameters(predicted_features, support)
                
                
                rule_list, support_list, confidence_list, element1_list, element2_list = TagsTraining.generate_association_rules(D, confidence, L1, support)
                
                rule_df = pd.DataFrame(columns=['Rule', 'Support', 'Confidence', 'element1', 'element2'])
                
                
                rule_df['Rule'] = rule_list
                rule_df['Support'] = support_list
                rule_df['Confidence'] = confidence_list
                rule_df['element1'] = element1_list
                rule_df['element2'] = element2_list
                
                
                with open("models/tagging/association_rules_with_resolution.pkl", "wb") as f:
                    pickle.dump(rule_df, f)
                
                
                # print("Association rules have been prepared and saved as association_rules_with_resolution.pkl file in the current directory.")
            ...
            tag_based_field.extend(['category', 'subcategory','description'])
    ...
    def loadCluster():
        ...
        try:
            ()
            cluster_doc = MongoDBPersistence.cluster_details_tbl.find_one({},{'_id':0, 'cluster_details':1})
            if(cluster_doc):
                response_lst = cluster_doc['cluster_details']
            else:
                model = joblib.load("models/tagging/model_clustering_3Fields.sav")
                number_of_features_to_be_taken = 20
                true_k = 14
                ...
                for i in range(true_k):
                    #print("Cluster %d:" % i),
                    dict_to_return = {'clusterName':'Cluster '+str(i)}
                    dict_to_return['values'] = []
                    for ind in order_centroids[i, :number_of_features_to_be_taken]:
                        tagg = total_terms[ind]
                        #print(' %s' % total_terms[ind]),
                        if len(total_terms[ind].split("_")) > 1:
                            tagg = "".join(total_terms[ind].split("_")[1:])

                        #dict_to_return['values'].append(total_terms[ind])
                        dict_to_return['values'].append(tagg)
                    response_lst.append(dict_to_return)

                logging.info('%s : Trying to insert cluster details'%RestService.timestamp())
                MongoDBPersistence.cluster_details_tbl.insert({'customer_id':customer_id, 'dataset_id':dataset_id, 'cluster_details':response_lst})
                
            return json_util.dumps(response_lst)
        except Exception as e:
            logging.error('Could not load/fetch cluster details : error - %s'%str(e))
        return []
#        return "<br>".join(str(dict_to_return).split(":"))

    @staticmethod
    def saveClusterDetails(customer_id, dataset_id):
        try:
            ()
            clstr_doc = {'customer_id':customer_id, 'dataset_id':dataset_id}
            clstr_doc_lst = request.get_json()
            clstr_doc['cluster_details'] = clstr_doc_lst
            logging.info('%s : Trying to update cluster details table'%RestService.timestamp())
            MongoDBPersistence.cluster_details_tbl.update({'customer_id':customer_id, 'dataset_id':dataset_id},{'$set':clstr_doc},upsert=True)
            return 'success'
        except Exception as e:
            logging.error('Could not update cluster details : error : %s'%str(e))
        return 'failure'
