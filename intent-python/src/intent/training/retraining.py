...
from intent.jobscheduler import retrainjobscheduler
...
#from sklearn.Utilser import KMeans     #Can be used in future
...
class Retrain(object):
    ...
    def retrain(customer_id, dataset_id):
        # print(training_hist_tbl.find_one())
        ...
        JobSettings = MongoDBPersistence.datasets_tbl.find_one({"CustomerID": customer_id, "DatasetID": dataset_id},
                                            {"JobSettings": 1, "_id": 0})  # Fetch from TblDataset JobSettings
        if (JobSettings):
            max_untrained_setting = JobSettings['JobSettings']['MinUntrained']
        else:
            logging.info("%s: Job setting not found for the dataset(DatasetID:%d)." % (RestService.timestamp(), dataset_id))
        max_untrained = MongoDBPersistence.training_tickets_tbl.find({"CustomerID": customer_id, "DatasetID": dataset_id,
        ...
        if (approved_algoparam):
            ...
            default_algo_params = list(MongoDBPersistence.algo_tbl.find({}, {'AlgorithmName': 1, 'DefaultParametersSingle': 1, "_id": 0}))
        ...
        for pred_field in pred_field_list:
            ...
            model = Utils(training_tkt_df, customer_id, dataset_id, pred_field, in_field, training_status,additional_fields)
            # df,cust_name,dataset_name,pred_field,in_field,training_status
                ...
                for algo in user_param_choices['Algorithms']:
                    ...
                    try:
                        if (algo_name == 'XGBClassifier'):
                            ...
                            logging.info('%s: Started training with %s algorithm.' % (RestService.timestamp(), algo_name))
                            score_dict = model.classify(model=SVC(C=param_grid['C'], kernel=param_grid['kernel'], probability=True))
                            logging.info('%s: Model has been trained successfully using %s algorithm.' % (RestService.timestamp(), algo_name))
        ...
        try:
            ...
            MongoDBPersistence.training_tickets_tbl.update_many({'CustomerID': customer_id, "DatasetID": dataset_id},
                                             {"$set": {"TrainingFlag": 1}})
            logging.info(
                '%s: Accuracies recorded successfully into TblTraining. TrainingID: %d' % (RestService.timestamp(), training_id))
        except Exception as e:
            logging.error('%s: Error occurred: %s ' % (RestService.timestamp(), str(e)))
