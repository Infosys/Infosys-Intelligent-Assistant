...
class ServiceNow(object):
    ...
    def generateAutoapprovalGraph(customer_id, dataset_id):
		...
		#z = pd_data['auto_approve'].value_counts()
    ...
    def generateAutoapprovalByUser(customer_id, dataset_id):
        ...
        try:
           ...
           # p = sns.barplot(list2,list1)
            #p.set( xlabel = "AutoApprovalByUser", ylabel = "Count")
           # plt.savefig(vocab_path)
            #plt.figure()
           # plt.show()
    ...
    def genenerateTotaltickets(customer_id, dataset_id):
        ...
        try:
            ...
            # for key, val in zip(itertools.cycle(list1), a):
            #     result1(list1)
    ...
    def generateTotalTicketsFetchedReport(datefrom, dateto,category):
        ...
        try:
            ...
            # c.rename(columns = {'index':category}, inplace = True)
    ...
    def users():
        ...
        #a=list(MongoDBPersistence._tbl.find({},{"_id":0,"resource_name":1}))
        pd_data = pd.DataFrame(closed_lst)
        c = pd_data["predicted_assigned_to"].unique().tolist()
        # print(c)
        # list1=pd_data.values
        # print(list1)
        # b=list1.tolist()
        # print(b)
        # print(type(b))
    ...
    def generateOpenTicketsByAssignee(customer_id, dataset_id, dropdown):
        try:
            if dropdown == 'Select_All':
                # closed_lst=list(MongoDBPersistence.rt_tickets_tbl.find({"state": { "$in": [ "New", "In Progress" ] }}))
            ...
            # print('-----',closed_lst)
                ...
                # closed_lst=list(MongoDBPersistence.rt_tickets_tbl.find({"CustomerID": 1, "assigned_to":dropdown},{"number":1,"assigned_to":1}))
                pd_data = pd.DataFrame(closed_lst)
                z = pd_data['predicted_assigned_to'].value_counts()
                list1 = z.index.to_list()
    ...
    def GenerateAssigneeByAssignmentgroup(customerid,assignee):
        ...
        # Records = [tuple(x) for x in z.to_records(index=False)]
        list4 = []
        list5 = []
        for i in d :
            a = list(i)
