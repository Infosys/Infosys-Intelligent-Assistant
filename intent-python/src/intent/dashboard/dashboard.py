...
@app.route('/api/getAssignmentGroup/<string:teamname>', methods=['GET'])
def getAssignmentGroup(teamname):
    return Dashboard.get_assignment_group(teamname)
...
class Dashboard(object):
    ...
    @staticmethod
    def get_assignment_group(teamname):

        try:
            df = Dashboard.get_dataset(teamname=teamname)
            df = df[['assignment_group']]
            df = df['assignment_group'].unique()
