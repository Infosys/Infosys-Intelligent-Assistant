...
class JobScheduler(object):
    ...
    def CreateJob(customer_id):
        ...
        #settings = [{"DatasetID": 1 , "TimeInterval": 60, "MaxUntrained": 0}]  # TimeInterval->days
        # log_setup()
        for cron_setting in settings:
            try:
                ...
                logging.info('%s: Error while creating new job for the dataset(DatasetID: ).' % (
                    RestService.timestamp()))  # ,cron_setting["DatasetID"]))
                return 'failure'
    ...
    def GetJobs(customer_id):
        ...
        # return json_util.dumps(scheduler.get_jobs())
        jobs = []
