...
class ProvideAccessToIIA(object):
    ...
    def approval_email_send(incident_no):
        ...
        try:
            predict_ticket = MongoDBPersistence.predicted_tickets_tbl.find_one({'number': incidentNumber},{'_id': 0,'NamedEntities': 1})
            # predict_ticket = MongoDBPersistence.predicted_tickets_tbl.find_one({'number': incidentNumber},{'_id': 0,'NamedEntities': 1, 'description':1})
            if(predict_ticket):
                # user_mail_lst=re.findall('[\w\.-]+@[\w\.-]+',predict_ticket['description'] , re.I)
                # if len(user_mail_lst) > 0:
                #     user_mailid = user_mail_lst[0]
                ...
                if(resp == 'success'):
                    date = datetime.now()
                    # wrkflw_tckt_dtls_doc['sent_on']=str(date)
                    ...
                    MongoDBPersistence.workflow_tickets_tbl.update_one({'IncidentNo':incidentNumber},{'$set': wrkflw_tckt_dtls_doc},upsert=True)
                else:
                    #--failed to send mail--
                    pass
            return 'success'
    ...
    def approval_email_receive(incident_no):
        try: 
            ...
            wrkflw_tckt_dtls_doc = MongoDBPersistence.workflow_tickets_tbl.find_one({"IncidentNo":incidentNumber,'approve_reject_state':False})
                ...
                if index>0:
                            ...
                            if(len(match_lst) > 0):
                                ...
                                if match_lst[0] == incidentNumber:
                                    ...
                                    if ('Approved' or 'approved') in messages[index].Body.split('\r\n')[0]:
                                        ...
                                        status = 'rejected'
                                    logging.info('Trying to update workflow_tickets_tbl with approve_reject_state as true')
                                    MongoDBPersistence.workflow_tickets_tbl.update_one({'IncidentNo':match_lst[0]},{'$set':{'approve_reject_state':True}},upsert=True)
                                    return status
                        else:
                    ...
                    if(not wrkflw_tckt_dtls_doc['send_rmndr_state']):
                        if(last_mail_rcvd_on > wrkflw_tckt_dtls_doc['reminder_date']):
                            ...
                            MongoDBPersistence.workflow_tickets_tbl.update_one({'IncidentNo':incidentNumber},{'$set':{'send_rmndr_state':True}})
                    ...
                    # --logic to stop waiting for response mail after a specific time period--
                        ...
                        MongoDBPersistence.workflow_tickets_tbl.update_one({'IncidentNo':incidentNumber},{'$set':{'approve_reject_state':True}},upsert=True)
                        return 'rejected'
                MongoDBPersistence.workflow_tickets_tbl.update_one({'IncidentNo':incidentNumber},{'$set':{'chckd_mail_till':last_mail_rcvd_on}},upsert=True)
            else:
                logging.warn('No Tickets found from workflow_tickets_tbl which is having approve_reject_status as false')
                return "no_data"
        except Exception as e:
            logging.error('%s:%s'%(RestService.timestamp(),str(e)))
        return 'no_response'
    ...
    def insert_user_dtls(incident_no):
        try:
            ...
            MongoDBPersistence.users_tbl.insert_one({"UserID": user_id,'UserName':user_name, "TeamID": 1, "Role" : "Admin"})
            return 'success'
        except Exception as e:
            logging.error('%s'%str(e))
        return 'failure'
