...
class RPA_Resolution(object):
    ...
    def execute_AA_RPA(workflow_name, inArguments,script_type):
        try:
            ...
            #machine = envs['name']
            if(script_type=='RPA'):
                config_Details = MongoDBPersistence.rpa_configuration_tbl.find_one({'scriptName':workflow_name},{"_id":0})
            else:
                config_Details = MongoDBPersistence.rpa_diag_configuration_tbl.find_one({'scriptName':workflow_name},{"_id":0})
            ...
            while AA_status=="QUEUED" or AA_status=="PENDING_EXECUTION" or AA_status=="UPDATE":
                ...
                if AA_status!="QUEUED" and AA_status!="PENDING_EXECUTION" and AA_status!="UPDATE":
                    # AA_status=="COMPLETED" or AA_status=="FAILED" or AA_status=="Others"
    ...
    def SaveRPA1():
        try:
            ...
            if(script_type=='RPA'):
                ...
                if checkEntry:
                    MongoDBPersistence.rpa_configuration_tbl.update({'scriptName':dbDict['scriptName']},{'$set':{'content':dbDict['content']}})
                else:
                    dbDict['keyword']=[]
                ...
                content=json.loads(script)
                dbDict['content']=content
                checkEntry=MongoDBPersistence.rpa_diag_configuration_tbl.find_one({'scriptName':dbDict['scriptName']})
                if checkEntry:
                    MongoDBPersistence.rpa_diag_configuration_tbl.update({'scriptName':dbDict['scriptName']},{'$set':{'content':dbDict['content']}})
                else:
                    dbDict['keyword']=[]
    ...
    def saveRPADetails():  
        try:
            ...
            for script in scripts:
                ...
                MongoDBPersistence.rpa_configuration_tbl.update({'scriptName':script['scriptName']},{'$set':{'content':contents1}})
    ...
    def getDiagnosticRPA(name):
        ...
        try:
            scripts= MongoDBPersistence.rpa_configuration_tbl.find_one({'scriptName':name},{"_id":0})['diagnostic_script']
            # print(scripts)
            if(scripts):
                return json_util.dumps(scripts)
            else:
...
#            print('Failure')
    ...
    def getRPA_Arguments(script_type,script_name,incident_number):
        try:
            # if(script_name=="AutomationAnywhere_ResetUserPasswordSNOW.json"):
            #     snowRecord=MongoDBPersistence.rt_tickets_tbl.find_one({'number':incident_number})
            #     snowArgs={'user_id': {'type': snowRecord['caller_id']}, 'incident_number': {'type': incident_number}}
            #     return json_util.dumps(snowArgs)
            # elif(script_name=="AutomationAnywhere_ResetIIAPassword.json"):
            #     snowRecord=MongoDBPersistence.rt_tickets_tbl.find_one({'number':incident_number})
            #     snowArgs={'ticket_number': {'type': incident_number}}
            #     return json_util.dumps(snowArgs)
            # else:
            if(script_type=='RPA'):
                args= MongoDBPersistence.rpa_configuration_tbl.find_one({'scriptName':script_name},{"_id":0})['content']['InArguments'] 
                ...
                args= MongoDBPersistence.rpa_diag_configuration_tbl.find_one({'scriptName':script_name},{"_id":0})['content']['InArguments']
                incident_number_config = get_config('RPA')
                incident_number_list = incident_number_config['incident_number_column_name']
                incident_number_list = incident_number_list.split(",")
                args_dict_list = list(args.keys())
    ...
    def execute_UIPath_RPA(script_name, args, script_type):
        try:
            ...
            #machine = envs['name']
            if(script_type=='RPA'):
                config_Details = MongoDBPersistence.rpa_configuration_tbl.find_one({'scriptName':script_name},{"_id":0})
            else:
                config_Details = MongoDBPersistence.rpa_diag_configuration_tbl.find_one({'scriptName':script_name},{"_id":0})
            ...
            # str1='\n{}:{}'.format(k,v)
            # for k,v in finalOutDict:
                
            # resString=json_util.dumps(finalOutDict)  
            return json_util.dumps(finalOutDict)
