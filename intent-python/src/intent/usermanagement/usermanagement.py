...
from intent.sso.sso import SSOAuthentication
    ...
    try:
        if request.method == 'GET':
            ...
            if sso_result[0]['Status'] == 'Success':
                user_status = json_util.loads(UserManagement.validateUser(sso_result[0]['UserId'], password='', sso=True))
...
class UserManagement(object):
    ...
    def validateUser(username, password='', sso=False):
        ...
        user_details = MongoDBPersistence.users_tbl.find_one({"UserID": user}, {"_id": 0, "UserID":1, "Role":1, 'Password':1, 'TeamID':1})
        if (user_details):
            # logging.info("Creating session of logged in User")
            # session["user"] = user_details["UserID"]
            if bool(sso):
                result = True
                pass
            else:
    ...
    def resetPassword():
        ...
        if (user_details):
            user_pass_details = MongoDBPersistence.users_tbl.find_one({"UserID": user_id, "Password": {"$exists":True}}, {"_id":0})
            if(user_pass_details):
                ...
                if (match):
                    MongoDBPersistence.users_tbl.update_one({"UserID": user_id}, {"$set":{"Password": hashed_new_password}})
    ...
    def addUser():
        ...
        check_user_id = list(MongoDBPersistence.users_tbl.find({'UserID' : user_id},{'_id':0,'TeamID':1,'UserID':1,'Role':1}))
        ...
        if len(check_user_id)>0 :
            ...
            status = MongoDBPersistence.users_tbl.insert_one({"UserID": user_id, "UserName":user_name, "TeamID": team_id, "Role": role})
    ...
    def getUsers():
        try:
            ...
            # users_lst = list(MongoDBPersistence.users_tbl.find({'UserID':{'$ne':'admin'}},{'_id':0, 'UserID':1, 'UserName': 1, 'Role':1}))
            if(users_lst):
